...
const traceStore = {
  ...
  _setupAutoSave() {
    ...
    this._state.autoSaveTimer = setInterval(() => {
      this._saveToStorage();
    }, this.config.autoSaveInterval || 5000);
  },
...
const createStore = (options = {}) => {
  ...
  const newStore = {
    // 复制配置和内部状态
    config: JSON.parse(JSON.stringify(traceStore.config)),
    _state: {
      store: {},
      listeners: {},
      autoSaveTimer: null,
    ...
    init: function(initOptions = {}) {
      // 合并options和initOptions
      const mergedOptions = { ...options, ...initOptions };
      
      // 如果提供了localStorageKey，使用它
      if (mergedOptions.localStorageKey) {
        this.config.localStorageKey = mergedOptions.localStorageKey;
      }
      
      // 应用其他配置
      if (mergedOptions.enableAutoSave !== undefined) {
        this.config.enableAutoSave = mergedOptions.enableAutoSave;
      }
      
      if (mergedOptions.autoSaveInterval) {
        this.config.autoSaveInterval = mergedOptions.autoSaveInterval;
      }
      
      if (mergedOptions.persistentKeys) {
        this.config.persistentKeys = mergedOptions.persistentKeys;
      }
      
      if (mergedOptions.logChanges !== undefined) {
        this.config.logChanges = mergedOptions.logChanges;
      }
  ...
  // 复制traceStore的所有方法到新实例
  const methods = Object.getOwnPropertyNames(traceStore).filter(
    prop => typeof traceStore[prop] === 'function' && prop !== 'init'
  );
  
  methods.forEach(method => {
    newStore[method] = traceStore[method];
  });
 ...
 * 创建一个默认的全局状态管理实例
 */
const traceStoreInstance = createStore({
  name: 'trace-store',
  enablePersist: true,
  enableAutoSave: true,
  persistKey: 'trace-app-state'
});

// 默认导出创建的实例
// export default traceStoreInstance;

// createStore已经作为const变量导出，不需要重复导出
// // export { createStore }; 
// CommonJS导出
module.exports = traceStoreInstance;
module.exports.createStore = createStore;
module.exports.traceStore = traceStore;
module.exports.createStore = createStore;
