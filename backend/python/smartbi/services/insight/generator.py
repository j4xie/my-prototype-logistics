...
from ..data.context_extractor import ContextInfo
...
7. 建议按优先级排序"""
    ...
    def _parse_llm_insights(
        ...
        Preserves executive_summary, risk_alerts, opportunities from LLM response
        by embedding them into a special metadata insight entry.
        """
        try:
            ...
            # Preserve structured fields that were previously discarded
            executive_summary = result.get("executive_summary", "")
            risk_alerts = result.get("risk_alerts", [])
            opportunities = result.get("opportunities", [])

            if executive_summary or risk_alerts or opportunities:
                valid_insights.insert(0, {
                    "type": "_meta",
