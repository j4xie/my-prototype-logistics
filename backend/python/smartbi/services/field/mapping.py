...
"""
Field Mapping Service (Fast Dictionary-Based)
...
This is the FAST path for field mapping - no LLM calls.
For LLM-based semantic mapping with caching, use SemanticMapper from semantic_mapper.py.

Design pattern:
- Use FieldMappingService for quick dictionary lookups
- Use SemanticMapper when LLM inference is needed
- They can be used together: FieldMappingService first, SemanticMapper for unmatched
...
from ..data.feature_analyzer import DataFeatureResult, DataType, NumericSubType
