...
async def test_writes_fact_rows_and_summary():
    ...
    assert summary_args[4] == 3.0
    # total_count
    assert summary_args[5] == 2
    # positive (rating >= 4) count = 1
    assert summary_args[6] == 1
...
async def test_unresolved_product_appended_to_unmatched_names():
    ...
    summary_args = conn.execute.await_args.args
    unmatched_jsonb = summary_args[9]
...
async def test_sentiment_rating_3_neutral_only_counted_in_total():
    ...
    summary_args = conn.execute.await_args.args
    assert summary_args[5] == 2  # total
    assert summary_args[6] == 0  # positive
    assert summary_args[7] == 0  # negative
    assert summary.rows_written == 2
...
async def test_no_rows_inserts_empty_summary():
    ...
    conn.execute.assert_awaited_once()
    summary_args = conn.execute.await_args.args
    assert summary_args[4] is None  # avg_rating None
    assert summary_args[5] == 0  # total_count 0
...
async def test_unmatched_names_dedup():
    ...
    summary_args = conn.execute.await_args.args
    unmatched = json.loads(summary_args[9])
