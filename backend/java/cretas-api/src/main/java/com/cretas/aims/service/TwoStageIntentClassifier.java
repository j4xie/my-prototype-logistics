...
public class TwoStageIntentClassifier {
    ...
    public enum ClassifiedDomain {
        ...
        SUPPLIER,    // 供应商, 供货商
        CUSTOMER,    // 客户, 买家
        FOOD,        // v15: 食品安全知识, HACCP, 添加剂, 标准法规
        UNKNOWN
    }
    ...
    static {
        ...
        // v15: 食品安全知识领域 — 用于食品知识库 RAG 查询
        DOMAIN_KEYWORDS.put(ClassifiedDomain.FOOD, Arrays.asList(
                // 食品安全体系与认证
                "HACCP", "GMP", "ISO22000", "FSSC22000", "BRC", "SQF", "食品安全",
                // 食品标准与法规
                "GB2760", "GB7718", "GB2762", "GB2761", "食品标准", "食品法规", "SC证",
                // 食品加工技术
                "巴氏杀菌", "UHT", "HTST", "CIP清洗", "杀菌工艺", "水活度", "灭菌",
                // 食品添加剂与安全
                "食品添加剂", "添加剂", "防腐剂", "着色剂", "甜味剂", "抗氧化剂",
                "黄曲霉毒素", "农药残留", "重金属", "兽药残留",
                // 食品品类知识
                "乳制品", "肉制品", "速冻食品", "预制菜", "婴幼儿食品", "保健食品",
                "有机食品", "冷链管理", "冷链", "食品包装", "食品标签",
                // 食品安全管理
                "虫害管理", "IPM", "食品追溯", "食品召回", "过敏原", "食品检测",
                "食品防护", "供应商审核", "食品卫生",
                // 微生物与污染物
                "大肠杆菌", "沙门氏菌", "金黄色葡萄球菌", "李斯特菌", "微生物"
        ));
    }
    ...
    private DomainResult classifyDomainWithKeyword(String input) {
        ...
        if (foodKeywords != null) {
            for (String keyword : foodKeywords) {
                if (input.contains(keyword)) {
                    log.debug("v15 FOOD domain priority match: '{}' -> FOOD", keyword);
        ...
        for (Map.Entry<ClassifiedDomain, List<String>> entry : DOMAIN_KEYWORDS.entrySet()) {
            if (entry.getKey() == ClassifiedDomain.FOOD) continue; // Already checked above
    ...
    /**
     * v19: 否定前缀列表 — 出现在动作词前时，将UPDATE语义翻转为QUERY
     * 例："未完成" → 状态查询（QUERY），非"完成"操作（UPDATE）
     */
    private static final List<String> NEGATION_PREFIXES = Arrays.asList(
            "未", "没有", "没", "不", "非", "无", "尚未", "还没", "还未"
    );

    /**
     * Check if input contains action/update words
     * v19: 添加否定前缀检测 — "未完成"、"没处理" 等不视为UPDATE动作
     */
    private boolean hasActionWords(String input) {
        for (String word : ACTION_WORDS) {
            ...
            if (idx >= 0) {
                // v19: 检查动作词前方是否有否定前缀
                if (hasNegationBefore(input, idx)) {
                    log.debug("v19 否定前缀检测: '{}' 前有否定词，不视为UPDATE动作", word);
                    continue; // 跳过此动作词，检查下一个
                }
                return true;
            }
        }
     ...
     * 向前搜索最多3个字符的范围
     */
    private boolean hasNegationBefore(String input, int wordStartIdx) {
        for (String neg : NEGATION_PREFIXES) {
            int negLen = neg.length();
            int negStart = wordStartIdx - negLen;
            if (negStart >= 0) {
                String before = input.substring(negStart, wordStartIdx);
                if (before.equals(neg)) {
                    return true;
                }
            }
        }
    ...
    private ClassifiedDomain inferDomainFromIntent(String intentCode) {
        ...
        // v15: Food knowledge domain
        if (upperIntent.startsWith("FOOD") || upperIntent.startsWith("FOOD_KNOWLEDGE")) {
            return ClassifiedDomain.FOOD;
        }
