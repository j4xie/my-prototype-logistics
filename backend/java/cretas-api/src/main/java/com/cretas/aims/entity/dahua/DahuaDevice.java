...
/**
 * 大华设备实体
 * 管理大华 IPC/NVR/DVR/XVR 设备
 *
 * @author Cretas Team
 * @since 2026-01-23
 */
@Entity
@Table(name = "dahua_devices",
        indexes = {
                @Index(name = "idx_dahua_factory_status", columnList = "factory_id, status"),
                @Index(name = "idx_dahua_ip_port", columnList = "ip_address, port"),
                @Index(name = "idx_dahua_serial", columnList = "serial_number")
        },
        uniqueConstraints = {
                @UniqueConstraint(name = "uk_dahua_factory_ip_port",
                        columnNames = {"factory_id", "ip_address", "port", "deleted_at"})
        })
@SQLDelete(sql = "UPDATE dahua_devices SET deleted_at = NOW() WHERE id = ?")
...
public class DahuaDevice extends BaseEntity {
    ...
    /**
     * 获取统一设备类型
     * 将大华 deviceType 映射到新的 UnifiedDeviceType
     *
     * @return 统一设备类型
     */
    @Transient
    public UnifiedDeviceType getUnifiedDeviceType() {
        return UnifiedDeviceType.fromDahuaDeviceType(this.deviceType);
    }

    /**
     * 设置统一设备类型
     * 自动转换为大华 deviceType 以保持兼容
     *
     * @param unifiedType 统一设备类型
     */
    public void setUnifiedDeviceType(UnifiedDeviceType unifiedType) {
        this.deviceType = unifiedType != null ? unifiedType.toDahuaDeviceType() : null;
    }
    ...
    /**
     * 大华设备状态枚举
     * 注意: 大华设备有 UNACTIVATED 状态，表示设备尚未激活配置
     */
    public enum DeviceStatus {
        ...
        ERROR,        // 错误
        UNKNOWN,      // 未知
        UNACTIVATED   // 未激活 (大华特有，设备未配置初始密码)
    }

    // ==================== 便捷方法 ====================

    /**
     * 获取设备基础URL (HTTP/HTTPS)
     */
    ...
    public String getTcpConnectionUrl() {
        return String.format("%s:%d", ipAddress, tcpPort);
    }
    ...
    public void markUnactivated() {
        this.status = DeviceStatus.UNACTIVATED;
        this.lastError = "设备未激活，需要配置初始密码";
    }
    ...
    public boolean isActivated() {
        return this.status != DeviceStatus.UNACTIVATED;
    }
