...
import com.cretas.aims.dto.intent.ActionType;
...
public class AIIntentServiceImpl implements AIIntentService {
            ...
            if (earlyPhraseMatch.isPresent()) {
                ...
                if (hasConflict) {
                    ...
                    if (intentOpt.isPresent()) {
                        ...
                        double phraseConfidence = 0.98;

                        // ========== v21.0: 歧义信号检测 + 置信度校准 ==========
                        // 检测可能导致 PhraseMatch 出错的歧义信号
                        IntentKnowledgeBase.AmbiguitySignals ambiguity =
                                knowledgeBase.detectAmbiguitySignals(userInput, matchedIntentCode);
                        ...
                        if (ambiguity.hasAmbiguity()) {
                            ...
                            log.info("v21.0 歧义检测: intent={}, 原置信度={}, 校准后={}, 信号={}",
                                    matchedIntentCode, String.format("%.2f", phraseConfidence),
                                    String.format("%.2f", calibratedConfidence), ambiguity);
                        }

                        // v21.0: 校准后置信度仍然很高，直接返回
                        if (calibratedConfidence >= 0.85) {
                            log.info("v21.0 PhraseMatch shortcut: input='{}', intent={}, confidence={}",
                        ...
                        if (calibratedConfidence >= matchingConfig.getLlmRerankingLowerBound()) {
                            log.info("v21.0 置信度降级触发 LLM Reranking: intent={}, calibrated={}, original={}",
                                    matchedIntentCode, String.format("%.2f", calibratedConfidence),
                            ...
                            // 添加可能的替代意图作为候选
                            addAlternativeCandidatesForReranking(candidates, matchedIntentCode, detectedActionType, allIntents);
                        ...
                        // v21.0: 校准后置信度很低 (< 0.58)，跳过 PhraseMatch
     ...
     *
     * 当 PhraseMatch 触发歧义检测后，根据检测到的 ActionType 和意图前缀，
     * 添加可能的替代意图作为候选，供 LLM 进行二次确认
     *
     * @param candidates 候选列表（将被修改）
     * @param matchedIntentCode 原始匹配到的意图代码
     * @param detectedActionType 检测到的操作类型
     * @param allIntents 所有可用意图列表
     */
    private void addAlternativeCandidatesForReranking(List<CandidateIntent> candidates,
                                                       String matchedIntentCode,
                                                       ActionType detectedActionType,
                                                       List<AIIntentConfig> allIntents) {
        // 提取意图前缀（如 PROCESSING_BATCH_LIST -> PROCESSING_BATCH）
        String intentPrefix = extractIntentPrefix(matchedIntentCode);
        if (intentPrefix == null) {
            return;
        }

        // 根据 ActionType 添加对应的候选意图
        Set<String> existingCodes = candidates.stream()
                .map(c -> c.getIntentCode())
                .collect(Collectors.toSet());

        // 根据检测到的 ActionType 添加对应的意图后缀
        switch (detectedActionType) {
            case CREATE:
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_CREATE", allIntents, 0.70);
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_ADD", allIntents, 0.65);
                break;
            case UPDATE:
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_UPDATE", allIntents, 0.70);
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_MODIFY", allIntents, 0.65);
                break;
            case DELETE:
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_DELETE", allIntents, 0.70);
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_CANCEL", allIntents, 0.65);
                break;
            case QUERY:
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_QUERY", allIntents, 0.70);
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_LIST", allIntents, 0.65);
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_STATS", allIntents, 0.60);
                break;
            case START:
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_START", allIntents, 0.70);
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_BEGIN", allIntents, 0.65);
                break;
            default:
                // 对于 UNKNOWN 或 AMBIGUOUS，添加常见的操作意图
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_LIST", allIntents, 0.60);
                addIntentByPattern(candidates, existingCodes, intentPrefix, "_CREATE", allIntents, 0.55);
                break;
        }

        log.debug("v21.0 为 LLM Reranking 添加了 {} 个替代候选", candidates.size() - 1);
    }

    /**
     * v21.0: 从意图代码中提取前缀
     * 例如：PROCESSING_BATCH_LIST -> PROCESSING_BATCH
     */
    private String extractIntentPrefix(String intentCode) {
        if (intentCode == null) {
            return null;
        }
        int lastUnderscore = intentCode.lastIndexOf('_');
        if (lastUnderscore > 0) {
            return intentCode.substring(0, lastUnderscore);
        }
        return intentCode;
    }
     ...
     * v21.0: 根据模式添加候选意图
     */
    private void addIntentByPattern(List<CandidateIntent> candidates,
                                    Set<String> existingCodes,
                                    String prefix,
                                    String suffix,
                                    List<AIIntentConfig> allIntents,
                                    double confidence) {
        String targetCode = prefix + suffix;
        if (existingCodes.contains(targetCode)) {
            return;
        }

        allIntents.stream()
                .filter(i -> i.getIntentCode().equals(targetCode))
                .findFirst()
                .ifPresent(intent -> {
                    candidates.add(CandidateIntent.fromConfig(
