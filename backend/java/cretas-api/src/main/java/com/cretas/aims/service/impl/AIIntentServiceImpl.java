...
public class AIIntentServiceImpl implements AIIntentService {
    ...
    public void setDisambiguationService(IntentDisambiguationService disambiguationService) {
        ...
        log.info("[Disambiguation] IntentDisambiguationService 注入: {}",
                disambiguationService != null ? "成功" : "未配置");
    }
    ...
    public IntentMatchResult recognizeIntentWithConfidence(String userInput, String factoryId, int topN,
        ...
        // ========== v22.0: 写操作前置检测 ==========
        // 如果动词+名词消歧检测到写操作(CREATE/UPDATE/DELETE)且置信度足够，
        // 绕过短语匹配（避免名词主导匹配覆盖写入动词）
        // 例: "创建订单" → verb+noun检测到CREATE+订单→ORDER_NEW，而短语匹配只会看到"订单"→ORDER_LIST
        if (verbNounResult != null && verbNounResult.isDisambiguated()
            ...
            // v22.1: 否定形式检测 - "未处理/没处理/没有处理" 不是写入动词
            ...
            if (detectedVerb != null) {
                ...
                if (verbIdx > 0) {
                    ...
                    isNegatedVerb = prefix.endsWith("未") || prefix.endsWith("没")
                            || prefix.endsWith("没有") || prefix.endsWith("非");
                }
            }
            if (!isNegatedVerb && (recAction == ActionType.CREATE || recAction == ActionType.UPDATE
                    || recAction == ActionType.DELETE)) {
                List<AIIntentConfig> allIntents = getAllIntents(factoryId);
                Optional<AIIntentConfig> intentOpt = allIntents.stream()
                        .filter(i -> i.getIntentCode().equals(recIntent))
                        .findFirst();
                if (intentOpt.isPresent()) {
                    AIIntentConfig intent = intentOpt.get();
                    log.info("v22.0 写操作前置: input='{}', intent={}, verb={}, noun={}, action={}",
                            userInput, recIntent, verbNounResult.getVerb(),
                            verbNounResult.getNoun(), recAction);
        ...
        if (!skipPhraseShortcut) {
            ...
            if (earlyPhraseMatch.isPresent()) {
                ...
                if (hasConflict) {
                    // v19: 冲突时用LLM消歧判断，而非无条件跳过
                    if (!"FOOD_KNOWLEDGE_QUERY".equals(matchedIntentCode) && disambiguationService != null) {
                        IntentMatchResult disambigResult = tryDisambiguateConflict(
                                userInput, matchedIntentCode, factoryId, preprocessedQuery);
                        if (disambigResult == null) {
                            // LLM判断为工厂数据→保留短语匹配
                            log.info("v19 短语冲突消歧: input='{}' LLM判断为工厂数据，保留短语匹配intent={}", userInput, matchedIntentCode);
            ...
            try {
                ...
                if (routeDecision != null) {
                    ...
                    if (routeDecision.canDirectExecute()) {
                        ...
                        // v18.0: 反向冲突检测 — 语义路由匹配到FOOD_KNOWLEDGE但输入含数据查询指标
                        boolean reverseSemanticConflict = "FOOD_KNOWLEDGE_QUERY".equals(routedIntent)
                                && knowledgeBase.containsDataQueryIndicator(userInput);
                        if (reverseSemanticConflict) {
                            log.info("v18.0 SemanticRouter反向冲突: input='{}' 语义路由到 FOOD_KNOWLEDGE 但含数据查询指标，跳过语义路由",
                                    userInput);
                            // Fall through to classifier/LLM for deeper matching
                        } else {
     ...
     * v17.0: LLM 消歧冲突处理
     *
     * 当 hasEntityIntentConflict() 检测到食品实体与工厂意图冲突时，
     * 调用 LLM 判断真实意图。
     *
     * @param originalInput 原始用户输入
     * @param matchedIntent 系统匹配到的意图代码
     * @param factoryId 工厂ID
     * @param preprocessedQuery 预处理查询（可为 null）
     * @return 如果应跳转到 FOOD_KNOWLEDGE_QUERY 则返回结果；如果保留原意图则返回 null
     */

    /**
     * v26g: 动词感知后处理 — 检测3类写入动词并override只读意图到对应写入意图
     * 1. 创建动词 (新建/录入/创建...) → _CREATE / _EXECUTE
     * 2. 删除动词 (删除/删掉/取消/移除...) → _CANCEL / _DELETE / _RELEASE
     * 3. 审批动词 (批准/同意/拒绝/驳回...) → ORDER_APPROVAL / ORDER_UPDATE
     */
