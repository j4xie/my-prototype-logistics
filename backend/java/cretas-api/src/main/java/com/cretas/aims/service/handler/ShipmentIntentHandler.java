...
public class ShipmentIntentHandler implements IntentHandler {
    ...
    private IntentExecuteResponse handleShipmentDelete(String factoryId, IntentExecuteRequest request,
        ...
        String shipmentId = null;
        if (request.getContext() != null) {
            shipmentId = getStringFromContext(request.getContext(), "shipmentId");
        }
        if (shipmentId == null && request.getUserInput() != null) {
            shipmentId = extractShipmentNumber(request.getUserInput());
        }
        if (shipmentId == null) {
            return IntentExecuteResponse.builder()
                    .intentRecognized(true).intentCode(intentConfig.getIntentCode())
                    .intentCategory("SHIPMENT").status("NEED_MORE_INFO")
                    .message("请提供要删除的出货单ID或出货单号 (shipmentId)")
                    .executedAt(LocalDateTime.now()).build();
        }

        // Look up by shipment number if not a direct ID
        Optional<ShipmentRecord> record = shipmentRecordService.getByShipmentNumber(shipmentId);
        if (record.isEmpty()) {
            record = shipmentRecordService.getByIdAndFactoryId(shipmentId, factoryId);
        }
        if (record.isEmpty()) {
            return IntentExecuteResponse.builder()
                    .intentRecognized(true).intentCode(intentConfig.getIntentCode())
                    .intentCategory("SHIPMENT").status("FAILED")
                    .message("未找到出货单: " + shipmentId)
                    .executedAt(LocalDateTime.now()).build();
        }

        shipmentRecordService.deleteShipment(record.get().getId());
        ...
        return IntentExecuteResponse.builder()
                ...
                .sensitivityLevel(intentConfig.getSensitivityLevel())
                .status("COMPLETED")
                .message("出货单 " + record.get().getShipmentNumber() + " 已删除")
                .affectedEntities(List.of(
                        IntentExecuteResponse.AffectedEntity.builder()
                                .entityType("ShipmentRecord")
                                .entityId(record.get().getId())
