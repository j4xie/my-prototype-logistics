...
```bash
# 查看默认配置
curl -X GET "http://47.100.235.168:10010/api/mobile/F001/ai-quota-configs" \
...
```bash
curl -X POST "http://47.100.235.168:10010/api/mobile/F002/ai-quota-configs" \
...
```bash
curl -X PUT "http://47.100.235.168:10010/api/mobile/F001/ai-quota-configs/{configId}" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "quotaCost": 2,
...
```bash
curl -X DELETE "http://47.100.235.168:10010/api/mobile/F001/ai-quota-configs/{configId}" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```javascript
// 创建配置
const response = await fetch('http://47.100.235.168:10010/api/mobile/F001/ai-quota-configs', {
  method: 'POST',
  headers: {
...
- [API文档](http://47.100.235.168:10010/swagger-ui.html)
