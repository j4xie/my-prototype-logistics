...
export const EnhancedLoginScreen: React.FC<LoginScreenProps> = ({ navigation }) => {
  ...
  useEffect(() => {
    if (username.length > 2) {
      const identification = UserIdentificationService.identifyUser(username);
      // 可以在这里显示用户类型提示
    }
  }, [username]);
  ...
  const renderUserIdentificationHint = () => {
    if (!userIdentification || username.length < 3) return null;
    ...
    return (
      <View style={styles.identificationHint}>
        <Ionicons 
          name="person-circle" 
          size={16} 
          color={userIdentification.userType === 'platform' ? '#FF9F43' : '#4ECDC4'} 
        />
        <Text style={styles.identificationText}>
          识别为{UserIdentificationService.getUserTypeDisplayName(userIdentification.userType)}
          {userIdentification.confidence > 0.7 ? ' (高置信度)' : ' (需确认)'}
        </Text>
      </View>
    );
  };
