...
class ApiClient {
  ...
  constructor(config: Partial<ApiClientConfig> = {}) {
    ...
    // 混合API模式：部分Mock已禁用，初始化时设置健康状态
    this.mockHealthStatus = {
      available: true, // 对于保留的Mock API仍然可用
      lastCheck: Date.now(),
      handlers: 35, // 估计的保留handlers数量
      environment: 'mixed-api'
    };

    console.log(`[API Client] 混合API模式初始化:`);
    console.log(`- Mock配置:`, this.mockConfig);
    console.log(`- 真实API: auth, users, platform`);
    console.log(`- Mock API: farming, processing, logistics, sales, admin, trace, products`);
  }
  ...
  private buildUrl(endpoint: string): string {
    ...
    // 检查是否为真实API端点
    const isRealApiEndpoint = realApiEndpoints.some(realEndpoint => 
      endpoint.includes(realEndpoint) || endpoint.startsWith(realEndpoint.replace('/', ''))
    );

    if (isRealApiEndpoint) {
      // 真实API端点：直接使用配置的后端URL
      console.log(`[API Client] Using Real API for: ${endpoint}`);
      const realApiUrl = `${this.config.baseURL}${endpoint.startsWith('/') ? '' : '/'}${endpoint}`;
      console.log(`[API Client] Real API URL: ${realApiUrl}`);
      return realApiUrl;
    } else {
      // Mock API端点：让MSW拦截
      console.log(`[API Client] Using Mock API for: ${endpoint}`);
      const mockApiUrl = `${window.location.origin}/api${endpoint.startsWith('/') ? '' : '/'}${endpoint}`;
      console.log(`[API Client] Mock API URL: ${mockApiUrl}`);
      return mockApiUrl;
    }
  }
