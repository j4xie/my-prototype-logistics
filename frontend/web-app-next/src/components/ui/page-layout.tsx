...
const PageLayout = forwardRef<HTMLDivElement, PageLayoutProps>(
  (
    ...
    const getContentPadding = () => {
      const topPadding =
        isMobile && (title || showBack || rightContent || menuItems.length > 0)
          ...
          : '';
      const bottomPadding =
        isMobile && bottomTabs.length > 0 ? 'pb-[80px]' : '';
    ...
    // 内容区域样式
    const contentClasses = cn('flex-1', 'w-full', getContentPadding());
    ...
    return (
      <div ref={ref} className={containerClasses} {...props}>
        {/* 移动端导航栏 */}
        {isMobile &&
          (title || showBack || rightContent || menuItems.length > 0) && (
            <MobileNav
              items={navItems}
...
const Content = forwardRef<HTMLDivElement, ContentProps>(
  ({ children, className, padding = true, ...props }, ref) => {
    const contentClasses = cn('w-full', padding && 'p-4', className);
