...
> ⚠️ **Phase A audit Chat 5 follow-up correction (2026-05-09)**: The 3 datasource POST/preview/apply lines above (`POST /datasource/upload` + `GET /datasource/{id}/preview` + `POST /datasource/apply`) were originally tagged `// ported` — that label is wrong. These 3 endpoints' **Java side is itself a TODO stub** (per PR #45 / #49 / #50 backlog defer plan from 2026-05-01), and **customers have never called them** (0 frontend caller + 0 prod log hit per Chat 5 audit `docs/qa-audits/2026-05-09-nginx-python-coverage-cross-check.md`). Phase 2A did **not** actually port them. Phase 2A → T6.5 → Phase 3 full chain: see Chat G PR `#185` (Python contract-completeness stub) + PR #45 / #49 / #50 (Phase 3 backlog defer docs). Other `/datasource/*` lines (`/list`, `/{id}/fields`, `/{id}/history`) are genuinely ported and remain `// ported`.
