...
| 路径 | 受众 | 章节顺序 |
|---|---|---|
| A | 新员工首周 | 4 → 5 → 3 → 1 → 8 → 12 |
| B | 店长首月 | 5 → 1 → 2 → 13 → 17 → 22 |
| C | 老板月度回顾 | 2 → 5 → 13 → 17 → 11 → 6 |
...
| 2-5 (轻模板, 骨架) | 22 | ~300 | ~6600 |
| 附录 | 3 路径 | ~150 | ~450 |
| **合计 (初版)** | — | — | **~12500 行** |

后续 Tier 2-5 按使用反馈扩到重模板 production-grade, 终版预估 ~25000-30000 行.
...
复用 `category="operation_manual"` 不新建 category. 理由:

- `manual_chat.py:435` 的 `categories=["operation_manual"]` 过滤逻辑不变
- KB AI 仍按"操作手册类"统一检索, 不需要前端选 category
- 区分文档身份靠 `source` 字段 (展示在 sources 列表里)
...
## 9. 实施分期 (Phased delivery)

### Phase 1: 骨架版本 (本设计批准后立即动)

- 写完 25 章骨架 + 3 培训路径 (轻模板)
- 体量 ~6000-8000 行 HTML
- ingest 1 次 (atomic swap 与 factory + restaurant-metrics-glossary 一起 refresh)
- 工时估计: 1 个 session (3-5 hr)

### Phase 2: Tier 1 production-grade (Phase 1 ship 后)

- 1 (AI Query) / 2 (财务 PBI) / 3 (Excel) 三章扩到重模板
- 每章 1500-2000 行
- ingest 1 次
- 工时估计: 2-3 sessions (单章一次)

### Phase 3+: Tier 2-5 按使用反馈扩 (Phase 2 ship 后, 长期)

- 监控 KB query 日志 (`manual_chat` 入参), 找出哪几章被高频问但 chunks 命中差
- 优先扩那几章到重模板
- 工时: 长期, 每月 1 章节奏
...
### 10.2 中期 (Phase 2 ship 后 1 月)

- [ ] KB query 日志显示 Tier 1 三章 chunks 占总 retrieval 命中 ≥40%
- [ ] 至少 1 个新客户上线时 PM 不需要再做屏幕共享培训

### 10.3 长期 (Phase 3+ 持续)

- [ ] Tier 2-5 章节按 query 频率扩到重模板, 长期 25 章全部 production-grade
- [ ] 客户支持工时减少 (定性观察, 无硬指标)
...
| 风险 | 概率 | 应对 |
|---|---|---|
| 写完发现 Tier 1 章节模板不合适, 需要调 | 中 | Phase 1 骨架后先小范围内审, 看模板是否能承接产品复杂度. 不行就调模板再扩深度 |
| RES_3101_009 真客户在使用中, KB 答的内容跟实际产品有偏差 | 中 | Phase 1 骨架完后, 找客户实际 AI Query log 抽 5-10 条对比. 偏差大就改章节内容 |
| 25 章骨架一次写太长, 容易并发 session 串入 (Apr 28 事故 5b) | 高 | 严格用 `git commit -- file1 file2` --only 模式. 每写完 5 章 commit 一次 (里程碑式 commit, 规则 1) |
| Tier 1 production-grade 章节最终 8000+ 行, 单文件总 30000 行后浏览器加载慢 | 低 | restaurant-metrics-glossary 330KB 已 prove out 30000 行不是问题. 真慢再拆 |
| 培训路径 A/B/C 跟实际新员工 onboarding 流不匹配 | 中 | Phase 1 ship 后跟客户运营负责人对一次, 调路径顺序 |

---

## 12. Open Questions (设计完成前需澄清)

无 — 所有结构性决策已 in scope (Q1-Q7 答完). 实施细节里的边界 case (e.g. 某章具体 KPI 列哪几个) 由实施时自行判断, 不阻塞设计 sign-off.

---

## 13. Spec Self-Review (per brainstorming skill)

- [x] **Placeholder scan**: 无 TBD / TODO / 占位符. 所有章节有具体名字和模板.
- [x] **Internal consistency**: §3 章节清单与 §8 培训路径锚点匹配 (§4/5/3/1/8/12 等都在清单内). §6 ingester 改动与 §4.3 chunk title 自动生成一致.
- [x] **Scope check**: 25 章 + 3 路径 是大但有边界 (不含工厂 / 不含截图 / 不含真客户号). Phase 1 骨架先 ship 而非一次性写完, 防过度投资.
- [x] **Ambiguity check**:
  - "production-grade" 定义: §4.1 重模板的 8 sections 全部填齐, 不留 TODO
  - "skeleton/骨架": §4.2 轻模板 4 sections 都有内容, 但每节 1-2 段, 不深入
  - "高频问的章节": §11 中期成功指标里说"Tier 1 三章 chunks 占总 retrieval 命中 ≥40%" — 量化判定

---

## 14. 批准检查表 (用户审批前过一遍)

- [ ] 25 章清单接受 (3 ① 章节里的拆分/合并都符合预期)
- [ ] Tier 1 / Tier 2-5 模板差异接受 (不要求 25 章全部 production-grade)
- [ ] 单文件 HTML 接受 (不拆 5 个 / 25 个文件)
- [ ] 不嵌截图接受 (RAG 不能用图)
- [ ] 脱敏化数据接受 (不引 RES_3101_009 真号)
- [ ] Phased delivery 接受 (Phase 1 骨架 → Phase 2 Tier 1 深度 → Phase 3+ 反馈驱动扩)
- [ ] 文件名 `restaurant-product-manual.html` 接受
- [ ] ingester source/category 复用 `operation_manual` 接受
...
2. 批准后调用 `superpowers:writing-plans` skill 写实施 plan, 把 25 章骨架拆成 task 列表
