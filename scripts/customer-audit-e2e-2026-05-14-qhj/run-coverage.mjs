#!/usr/bin/env node
/**
 * R7-E1: RES_3101_009 (QHJ chain) 51-ask coverage replay — 2026-05-14
 *
 * Mirrors the F006 (六扇门) coverage audit pattern from
 * scripts/customer-audit-e2e-2026-05-13/run-coverage.mjs against the QHJ chain
 * data-bearing factory.
 *
 * Substitution rationale: R_QINGHUAJIAO_REAL was the spec'd target but has 1
 * user / **0** POS records (per Issue #538). RES_3101_009 is the same QHJ chain
 * with 140K+ POS records and a seeded qhj_prod factory_super_admin user. Steve
 * sign-off 2026-05-14.
 *
 * Account constraint vs F006: RES_3101_009 has only `qhj_prod`
 * (factory_super_admin). Cross-role RBAC scenarios (warehouse_mgr-only) cannot
 * be replayed on this factory and are emitted as SKIP_RBAC verdicts that
 * document the account-gap finding rather than assert behavior. F006 / GML
 * accounts retained so cross-factory negative scenarios (qhj_prod → F006 data
 * expect 403) can still run.
 *
 * 跑法:
...
const ACCOUNTS = {
  // RES_3101_009 (QHJ chain) — only factory_super_admin seeded.
  qhj_prod: { username: 'qhj_prod', password: '123456' },
  // F006 cross-role accounts retained for cross-factory negative checks and
  // for scenarios whose verdict on RES_3101_009 needs comparison evidence.
  // RBAC strip behavior on RES_3101_009 itself cannot be tested via
  // warehouse_mgr because that role is not seeded here.
...
// Scenarios whose F006-flavored `account` we remap to qhj_prod for this audit.
// Scenarios with account == 'f006_warehouse_mgr' are emitted SKIP_RBAC unless
// remapped here (no warehouse_mgr seeded on RES_3101_009).
const ACCOUNT_REMAP = { f006_admin: 'qhj_prod' };
...
async function login(page, account) {
  ...
  // R7-E1 fix: prod 139:8086 SPA loads a 588KB pdf-lib JS bundle that takes
  // ~5s to download (curl-verified). The original waitUntil:'load' in the
  // F006 script (and Playwright's default) waits for the load event which
  // never fires on this SPA — websocket/polling keeps it "loading" forever.
  // Use waitUntil:'commit' + targeted waitForSelector on the username input
  // so we resume as soon as the SPA hydrates (~6-8s typical).
  try {
    await page.goto(`${TARGET}/login`, { waitUntil: 'commit', timeout: 30000 });
  } catch (e) {
    console.error(`  ✗ login: goto /login failed for ${account}: ${e.message}`);
    return false;
  }
  try {
    await page.waitForSelector('input[placeholder*="用户名"], input[type="text"]', { timeout: 30000 });
  } catch {
    console.error(`  ✗ login: username input not found for ${account} within 30s`);
    return false;
  }
...
async function runScenario(browser, sc) {
  ...
  // R7-E1 adaptation: warehouse_mgr role not seeded on RES_3101_009. Emit
  // SKIP_RBAC instead of attempting login that would fail anyway. Other F006
  // accounts (admin) remap to qhj_prod via ACCOUNT_REMAP.
  const remappedAccount = ACCOUNT_REMAP[sc.account] || sc.account;
  if (remappedAccount === 'f006_warehouse_mgr') {
    return {
      scenario: sc.tag,
      group: sc.group,
      ...
      status: 'SKIP_RBAC',
      evidence: {
        reason: 'warehouse_mgr role not seeded on RES_3101_009; cross-role RBAC verification out of scope for this factory. Documented finding, not a failure.',
        seededAccounts: ['qhj_prod (factory_super_admin)'],
      },
      ts: new Date().toISOString(),
    };
  }
  const state = await getOrCreateStorageState(browser, remappedAccount);
  if (!state) {
    return { scenario: sc.tag, group: sc.group, priority: sc.priority, account: remappedAccount, status: 'ERROR', evidence: { error: `login failed for ${remappedAccount}` }, ts: new Date().toISOString() };
  }
  const ctx = await browser.newContext({ acceptDownloads: true, storageState: state });
  // R7-E1 fix: default 30s nav timeout occasionally races with the 588KB
  // pdf-lib SPA bundle (5s curl, more under chromium concurrency). Bump to
  // 60s so the 2 ERROR-timeouts observed in Group A run downgrade to real
  // INFO/PASS verdicts. Login path keeps its own 30s timeout because it uses
  // waitUntil:'commit' which fires on nav-commit, not resource-load.
  ...
  try {
    ...
    return { scenario: sc.tag, group: sc.group, priority: sc.priority, account: remappedAccount, originalAccount: sc.account, status: result.verdict || 'INFO', evidence: result, ts: new Date().toISOString() };
  } catch (err) {
    return { scenario: sc.tag, group: sc.group, priority: sc.priority, account: remappedAccount, originalAccount: sc.account, status: 'ERROR', evidence: { error: String(err) }, ts: new Date().toISOString() };
  } finally {
    await ctx.close();
  }
}
...
async function main() {
  ...
  const out = {
    ...
    statusCounts: {
      ...
      SKIP_RBAC: results.filter((r) => r.status === 'SKIP_RBAC').length,
      ERROR: results.filter((r) => r.status === 'ERROR').length,
    },
    factory: FACTORY_ID,
    substitutionNote: 'Originally spec\'d R_QINGHUAJIAO_REAL had 0 POS records; substituted RES_3101_009 (same QHJ chain, 140K+ POS, qhj_prod admin user) per Steve sign-off 2026-05-14.',
    results,
  };
  const outFile = path.join(__dirname, 'results-coverage.json');
  await fs.writeFile(outFile, JSON.stringify(out, null, 2), 'utf8');
  console.log(`\nResults written to ${outFile}`);
  console.log(`Summary: PASS=${out.statusCounts.PASS} FAIL=${out.statusCounts.FAIL} PARTIAL=${out.statusCounts.PARTIAL} INFO=${out.statusCounts.INFO} SKIP_RBAC=${out.statusCounts.SKIP_RBAC} ERROR=${out.statusCounts.ERROR}`);
}
