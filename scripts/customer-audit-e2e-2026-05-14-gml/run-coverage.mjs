 ...
 *
 * Adapted from `scripts/customer-audit-e2e-2026-05-13/run-coverage.mjs` (F006
 * template). Factory R_GML_DEMO is a RESTAURANT demo factory with ZERO data
 * across all major tables (sales_orders / purchase_orders / production_plans
 * / material_batches / raw_material_types / product_types / suppliers /
 * restaurant_sales_plans / return_orders / pos_order_syncs / production_*
 * all = 0 rows per prod DB query 2026-05-14).
 *
 * Expected outcome shape (different from F006 which had real factory data):
 *   - PASS — login, navigation, empty-state list rendering, console-clean
 *   - INFO — data-dependent scenarios that need rows to verify (most T3-x and T4-x)
 *   - N/A — factory-specific scenarios that don't apply to restaurant (BOM,
 *           procurement-detail, production-batch flow)
 *   - FAIL — only when an actual page crash / 5xx / console-error / RBAC leak
 *
 * The 51-ask count in PR title refers to the original F006 customer-ask matrix
 * which is the comparable axis. R_GML_DEMO will show many INFO/N/A by nature.
 *
 * Only one account verified: gml_admin / 123456 (role=factory_super_admin).
 * No warehouse_mgr or other-role seeded → cross-role RBAC checks not runnable
 * (mark RBAC scenarios as N/A or INFO-restaurant-no-role-seed).
 *
 * 跑法:
 *   node run-coverage.mjs --all                        # 全部 35 scenarios
 *   node run-coverage.mjs --group A|B|D|E              # 单组
 *   node run-coverage.mjs --tag S-COV-T3-5             # 单个
 *
 * 约束: 只读 prod 139:8086, 不写入.
 *
 * Output:
...
// R_GML_DEMO only has 1 seeded user (gml_admin, factory_super_admin role). The
// f006_admin/f006_warehouse_mgr entries are kept as keys so per-scenario
// `account: 'f006_admin'` references still resolve, but they actually route
// to the gml_admin login — every scenario from the F006 template runs as
// gml_admin on R_GML_DEMO data (which is empty, so most return INFO).
// This is the "same pattern" baseline per R7 Path E MO.
const ACCOUNTS = {
  f006_admin: { username: 'gml_admin', password: '123456' },
  f006_warehouse_mgr: { username: 'gml_admin', password: '123456' },
