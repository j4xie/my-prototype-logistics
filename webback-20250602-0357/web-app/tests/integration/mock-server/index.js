/**
 * @file tests/integration/mock-server/index.js
 * @description Mock服务器
 * 用于性能测试，提供可控延迟和带宽限制的资源响应
 * 
 * 用法: node web-app/tests/integration/mock-server/index.js [--port=9090] [--delay=200] [--bandwidth=1024]
 */
...
function handleStaticRequest(req, res, pathname) {
  // 默认提供测试资源
  const filePath = path.join(process.cwd(), 'web-app/tests/integration/mock-server/static', pathname);
...
// 创建静态目录（如果不存在）
const staticDir = path.join(process.cwd(), 'web-app/tests/integration/mock-server/static');
