...
public interface DynamicDataPersistenceService {
    ...
    /**
     * Persist parsed Excel data to PostgreSQL dynamic storage with original file name
     *
     * @param factoryId Factory ID for tenant isolation
     * @param parseResponse Parsed Excel data with field mappings
     * @param fileName Original Excel file name (e.g., "Test.xlsx")
     * @return Persistence result with uploadId and field definitions
     */
    DynamicPersistenceResult persistDynamic(String factoryId, ExcelParseResponse parseResponse, String fileName);
    ...
    /**
     * Persist data with confirmed field mappings and original file name
     *
     * @param factoryId Factory ID
     * @param parseResponse Parsed Excel data
     * @param confirmedMappings User-confirmed field mappings
     ...
     * Uses REQUIRES_NEW transaction to survive main transaction rollback.
     *
     * @param uploadId Upload record ID
    ...
    /**
     * Clear data and field definitions for an upload, keeping the upload record.
     * Used before retry to clean up partial data.
     *
     * @param uploadId Upload record ID
     */
    void clearDataByUploadId(Long uploadId);

    /**
     * Retry persistence using an existing upload record.
     * Clears old data, re-persists from parse response, and updates upload status.
     *
     * @param existingUploadId The existing upload record ID
     * @param factoryId Factory ID
     * @param parseResponse Parsed Excel data
     * @param fileName Original file name
     * @return Persistence result
     */
    DynamicPersistenceResult retryPersistDynamic(Long existingUploadId, String factoryId,
