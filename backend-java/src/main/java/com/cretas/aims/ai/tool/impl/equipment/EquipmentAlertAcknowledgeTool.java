 ...
 * 设备告警确认工具
 *
 * 用于确认（知晓）指定的设备告警。确认后告警状态变为ACKNOWLEDGED。
 * 表示用户已经知道这个告警，但尚未解决。
 *
 * @author Cretas Team
 * @version 1.0.0
...
@Slf4j
@Component
public class EquipmentAlertAcknowledgeTool extends AbstractBusinessTool {
    ...
    public String getToolName() {
        return "equipment_alert_acknowledge";
    }

    @Override
    public String getDescription() {
        return "确认（知晓）指定的设备告警。需要提供告警ID。确认后告警状态变为ACKNOWLEDGED，" +
    ...
    protected Map<String, Object> doExecute(String factoryId, Map<String, Object> params, Map<String, Object> context) throws Exception {
        log.info("执行设备告警确认 - 工厂ID: {}, 参数: {}", factoryId, params);
        ...
        result.put("nextSteps", Arrays.asList(
                "查看告警详情以了解问题根源",
                "制定解决方案",
                "解决后使用equipment_alert_resolve标记为已解决"
        ));

        log.info("设备告警确认完成 - 告警ID: {}, 确认人: {}", alertId, userName);
