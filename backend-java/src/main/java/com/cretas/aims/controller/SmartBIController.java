...
public class SmartBIController {
    ...
    public ResponseEntity<ApiResponse<ExcelParseResponse>> uploadExcel(
        ...
        try {
            ...
            if (headerRow == null || headerRow < 0) {
                headerRows = 0;  // Python auto-parse 会自动检测
                log.debug("使用 Python 自动检测表头行数");
            } else {
                headerRows = headerRow + 1;  // 前端从 0 开始，Python 从 1 开始
            }
    ...
    @Operation(summary = "重试失败的 Sheet", description = "重新解析和持久化失败或卡住的 Sheet 数据")
    public ResponseEntity<ApiResponse<Map<String, Object>>> retrySheet(
            @Parameter(description = "工厂ID") @PathVariable String factoryId,
            @Parameter(description = "上传记录ID") @PathVariable Long uploadId) {

        log.info("重试 Sheet 上传: factoryId={}, uploadId={}", factoryId, uploadId);

        if (uploadFlowService == null) {
            return ResponseEntity.ok(ApiResponse.error("上传服务不可用"));
        }
        ...
        try {
            ...
            if (result.isSuccess()) {
                ...
                return ResponseEntity.ok(ApiResponse.success("重试成功", data));
            } else {
                return ResponseEntity.ok(ApiResponse.error(result.getMessage()));
            }
        } catch (Exception e) {
            log.error("重试 Sheet 失败: {}", e.getMessage(), e);
