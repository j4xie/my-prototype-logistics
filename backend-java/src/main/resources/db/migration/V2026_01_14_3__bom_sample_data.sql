...
-- 插入 BOM 项目示例数据 (如果不存在)
INSERT IGNORE INTO bom_items (factory_id, product_type_id, material_type_id, material_name, standard_quantity, yield_rate, unit, unit_price, tax_rate, sort_order, notes)
SELECT
    'F001' as factory_id,
...
-- 插入人工费用配置示例 (如果不存在)
INSERT IGNORE INTO labor_cost_configs (factory_id, product_type_id, process_name, process_category, unit_price, price_unit, standard_quantity, sort_order, notes)
VALUES
...
-- 插入均摊费用配置 (如果不存在)
INSERT IGNORE INTO overhead_cost_configs (factory_id, name, category, unit_price, price_unit, allocation_rate, sort_order, notes)
SELECT * FROM (
