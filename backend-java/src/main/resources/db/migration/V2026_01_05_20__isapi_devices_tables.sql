...
CREATE TABLE IF NOT EXISTS isapi_devices (
    ...
    supports_ptz TINYINT(1) DEFAULT 0 COMMENT '是否支持PTZ',
    supports_audio TINYINT(1) DEFAULT 0 COMMENT '是否支持音频',
    supports_smart TINYINT(1) DEFAULT 0 COMMENT '是否支持智能分析',
    ...
    -- 订阅状态
    alert_subscribed TINYINT(1) DEFAULT 0 COMMENT '是否已订阅告警',
...
CREATE TABLE IF NOT EXISTS isapi_event_logs (
    ...
    picture_data MEDIUMBLOB COMMENT '抓拍图片二进制数据',

    -- 时间戳
    event_time DATETIME NOT NULL COMMENT '事件发生时间 (设备时间)',
    received_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '服务器接收时间',
...
CREATE TABLE IF NOT EXISTS isapi_device_channels (
    ...
    -- 配置
    recording_enabled TINYINT(1) DEFAULT 0 COMMENT '是否启用录像',
    smart_enabled TINYINT(1) DEFAULT 0 COMMENT '是否启用智能分析',
