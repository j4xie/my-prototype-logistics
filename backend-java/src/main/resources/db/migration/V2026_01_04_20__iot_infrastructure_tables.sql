...
COMMENT='IoT设备注册表 - 管理工厂内所有IoT设备的注册与配置信息';
...
CREATE TABLE IF NOT EXISTS iot_device_data (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '数据ID，自增主键',
    ...
    -- 时间戳
    collected_at DATETIME NOT NULL COMMENT '数据采集时间(设备端时间)',
    received_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '数据接收时间(服务端时间)',
...
COMMENT='IoT设备数据采集表 - 高频存储设备实时上报数据';
...
CREATE TABLE IF NOT EXISTS iot_device_alerts (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '告警ID，自增主键',
    device_id VARCHAR(36) NOT NULL COMMENT '设备ID (iot_devices.id)',
...
COMMENT='IoT设备告警表 - 记录设备运行告警与处理状态';
...
CREATE TABLE IF NOT EXISTS iot_device_heartbeats (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '心跳记录ID',
...
COMMENT='IoT设备心跳记录表 - 记录设备心跳历史用于稳定性分析';
...
CREATE TABLE IF NOT EXISTS iot_device_commands (
    ...
    expire_at DATETIME COMMENT '命令过期时间',
...
COMMENT='IoT设备命令表 - 记录下发给设备的命令及执行状态';
