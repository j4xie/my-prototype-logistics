...
CREATE TABLE IF NOT EXISTS factory_scheduling_config (
    ...
    -- 权重参数
    linucb_weight DOUBLE DEFAULT 0.60 COMMENT 'LinUCB分数权重',
    fairness_weight DOUBLE DEFAULT 0.15 COMMENT '公平性加分权重',
    skill_maintenance_weight DOUBLE DEFAULT 0.15 COMMENT '技能维护加分权重',
    repetition_weight DOUBLE DEFAULT 0.10 COMMENT '重复惩罚权重',
    ...
    -- 临时工配置
    temp_worker_linucb_factor DOUBLE DEFAULT 0.7 COMMENT '临时工LinUCB权重调整因子',
    temp_worker_fairness_factor DOUBLE DEFAULT 1.5 COMMENT '临时工公平性权重调整因子',
    ...
    -- SKU复杂度配置
    sku_complexity_weight DOUBLE DEFAULT 0.15 COMMENT 'SKU复杂度权重',
    ...
    learning_rate DOUBLE DEFAULT 0.05 COMMENT '学习率',
    min_samples_for_adaptation INT DEFAULT 50 COMMENT '最小样本数',
    efficiency_target DOUBLE DEFAULT 0.85 COMMENT '效率提升目标',
    diversity_target DOUBLE DEFAULT 0.70 COMMENT '多样性目标',

    -- 异常检测配置
    anomaly_detection_enabled BOOLEAN DEFAULT TRUE COMMENT '是否启用异常检测',
    efficiency_anomaly_threshold DOUBLE DEFAULT 0.50 COMMENT '效率异常阈值',
    anomaly_count_for_calibration INT DEFAULT 3 COMMENT '触发校准的异常次数',

    -- 审计字段
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    last_adaptation_at DATETIME NULL COMMENT '最后自适应调整时间',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='工厂调度配置表';
...
CREATE TABLE IF NOT EXISTS factory_temp_worker (
    ...
    skill_growth_rate DOUBLE DEFAULT 0.0 COMMENT '技能成长率',

    -- 绩效追踪
    total_assignments INT DEFAULT 0 COMMENT '总分配次数',
    avg_efficiency DOUBLE DEFAULT 0.0 COMMENT '平均效率',
    reliability_score DOUBLE DEFAULT 0.5 COMMENT '可靠性评分 (出勤率等)',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='临时工记录表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='SKU复杂度配置表';
...
CREATE TABLE IF NOT EXISTS scheduling_adaptation_log (
    ...
    -- 调整前参数
    old_linucb_weight DOUBLE NULL,
    old_fairness_weight DOUBLE NULL,
    old_skill_maintenance_weight DOUBLE NULL,
    old_repetition_weight DOUBLE NULL,

    -- 调整后参数
    new_linucb_weight DOUBLE NULL,
    new_fairness_weight DOUBLE NULL,
    new_skill_maintenance_weight DOUBLE NULL,
    new_repetition_weight DOUBLE NULL,
    ...
    -- 调整时的指标
    current_efficiency DOUBLE NULL COMMENT '当前效率',
    current_diversity DOUBLE NULL COMMENT '当前多样性',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='自适应学习日志表';
