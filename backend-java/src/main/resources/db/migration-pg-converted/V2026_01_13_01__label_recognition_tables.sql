...
-- Original file: V2026_01_13_01__label_recognition_tables.sql
-- Conversion date: 2026-01-26 18:48:14
...
CREATE TABLE IF NOT EXISTS label_recognition_configs (
    ...
    -- 识别配置
    min_confidence DOUBLE PRECISION DEFAULT 0.7 COMMENT '最低置信度阈值',
    default_batch_id VARCHAR(100) COMMENT '默认关联批次ID',

    -- 状态
    enabled BOOLEAN DEFAULT TRUE COMMENT '是否启用',
    last_trigger_time TIMESTAMP WITH TIME ZONE COMMENT '最后触发时间',
...
CREATE TABLE IF NOT EXISTS label_recognition_records (
    ...
    confidence DOUBLE PRECISION COMMENT '识别置信度',
    quality_score DOUBLE PRECISION COMMENT '质量分数',
    quality_issues TEXT COMMENT '质量问题(JSON数组)',

    -- 图片数据
    captured_image BYTEA COMMENT '抓拍图片',
    captured_image_url VARCHAR(500) COMMENT '图片URL(OSS)',

    -- 时间
    recognition_time TIMESTAMP WITH TIME ZONE NOT NULL COMMENT '识别时间',
