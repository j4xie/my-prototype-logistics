...
-- Original file: V2025_12_30_4__factory_capacity_config.sql
-- Conversion date: 2026-01-26 18:46:15
...
CREATE TABLE IF NOT EXISTS factory_capacity_configs (
    ...
    -- 加班配置
    allow_overtime SMALLINT(1) NOT NULL DEFAULT 1 COMMENT '是否允许加班',
...
CREATE TABLE IF NOT EXISTS insert_slot_locks (
    ...
    -- 时段信息
    start_time TIMESTAMP WITH TIME ZONE NOT NULL COMMENT '时段开始时间',
    end_time TIMESTAMP WITH TIME ZONE NOT NULL COMMENT '时段结束时间',

    -- 锁定信息
    locked_by BIGINT NOT NULL COMMENT '锁定用户ID',
    locked_by_username VARCHAR(100) NULL COMMENT '锁定用户名',
    locked_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '锁定时间',
    expire_at TIMESTAMP WITH TIME ZONE NOT NULL COMMENT '锁定过期时间',

    -- 锁定原因/用途
    lock_purpose VARCHAR(50) NULL DEFAULT 'URGENT_INSERT' COMMENT '锁定用途: URGENT_INSERT/PREVIEW/EDITING',

    -- 元数据
    is_active SMALLINT(1) NOT NULL DEFAULT 1 COMMENT '是否有效',
    released_at TIMESTAMP WITH TIME ZONE NULL COMMENT '释放时间',
