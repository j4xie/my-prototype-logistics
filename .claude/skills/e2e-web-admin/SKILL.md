...
对每个模块：
1. 导航到列表页 → **列扫描** (L5.5)
2. 点"新建"按钮 → **字段类型验证** (L5.4)
3. 识别所有表单字段，填写必填字段
4. 提交，检查成功提示（无 error）
5. 刷新验证数据持久化 → **UUID扫描** (L5.1) + **非空检测** (L5.2)
6. 点"详情" → 逐项核对值 → **状态标签中文化** (L5.6)
7. **重复提交** (L5.3): 同数据第二次提交应被拒绝
...
| 2 CRUD | 创建/编辑/删除 | 填表单→提交→验证成功→**详情页核对值**→**UUID扫描**→**重复提交** |
| 3 跨模块 | 数据一致性 | A创建→B下拉能选到 |
| 4 链路 | 完整业务流程 | 端到端走通 |
| **5 (嵌入)** | 业务正确性 | **嵌入L2-L4执行，非独立阶段。** UUID+非空+重复+字段类型+计算+只读+递增+筛选 |
