...
// UPDATED CODE: 导出组件
export const TraceDataTools = {
    initImport: function(targetElement, formElement) {
        return createImportUI(targetElement, formElement);
    },
    
    initScan: function(formElement) {
        return addScanButtonsToForm(formElement);
    },
    
    init: function(targetElement, formElement) {
        this.initImport(targetElement, formElement);
        this.initScan(formElement);
        console.log('数据导入与扫描组件已初始化');
        return this;
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (importContainer && traceForm) {
        TraceDataTools.init(importContainer, traceForm);
    }
}); 