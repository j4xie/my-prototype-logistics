...
from .structure_detector import StructureDetectionResult, ColumnInfo
from .semantic_mapper import SemanticMappingResult, FieldMapping
from .context_extractor import ContextExtractor, ContextInfo
...
class FixedExecutor:
    ...
    def _smart_merge_headers(
        ...
        for row_idx in range(1, header_row_count + 1):
            ...
            is_title_row = len(unique_values) <= 2 and len(non_empty) > max_col * 0.3
            is_unit_row = any("单位" in v or "编制" in v or "Unit" in v.lower() for v in non_empty)
            is_date_row = len(non_empty) <= 3 and any("00:00:00" in v or len(v) == 10 for v in non_empty)
            ...
            if is_title_row:
                logger.debug(f"Row {row_idx} is title row, skipping")
                continue

            if is_unit_row:
                logger.debug(f"Row {row_idx} is unit row, skipping")
                continue

            if is_date_row:
                logger.debug(f"Row {row_idx} is date row, skipping")
                continue

            meaningful_rows.append((row_idx, row_values))
            logger.debug(f"Row {row_idx} is meaningful header row")
