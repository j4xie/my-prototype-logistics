        ...
        if r.status_code == 200:
            ...
            if data.get('code') == 200:
                ...
                detailed_results.append({
                    ...
                    'confidence': conf,
                    'method': method,
                    'intent': resp_data.get('intentCode', 'N/A')[:25] if resp_data.get('intentCode') else 'N/A'
                })
            else:
                results['failed'] += 1
        else:
...
print('Edge Cases (first 10):')
for r in detailed_results[:10]:
    print(f'  {r["input"]:30} conf={r["confidence"]:.2f} method={r["method"]:15} intent={r["intent"]}')
print('')
print('Domain-specific (sample):')
for r in detailed_results[37:47]:
    print(f'  {r["input"]:30} conf={r["confidence"]:.2f} method={r["method"]:15} intent={r["intent"]}')
print('')
print('Typos (sample):')
for r in detailed_results[57:67] if len(detailed_results) > 67 else detailed_results[57:]:
    print(f'  {r["input"]:30} conf={r["confidence"]:.2f} method={r["method"]:15} intent={r["intent"]}')
print('')
print('Multi-intent (sample):')
for r in detailed_results[77:] if len(detailed_results) > 77 else detailed_results[-10:]:
    print(f'  {r["input"]:30} conf={r["confidence"]:.2f} method={r["method"]:15} intent={r["intent"]}')
