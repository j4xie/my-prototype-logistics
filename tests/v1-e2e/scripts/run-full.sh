#!/usr/bin/env bash
# =============================================================================
# run-full.sh — Post-deploy full test orchestrator for the V1 E2E suite
# =============================================================================
# Usage:
#   bash tests/v1-e2e/scripts/run-full.sh              # All tests (Full Suite)
#   bash tests/v1-e2e/scripts/run-full.sh --pr-gate     # PR gate only (same as run-pr-gate.sh)
#
...
#   3. Runs ALL playwright tests (or --grep @pr-gate if --pr-gate flag passed)
...
echo -e "${BOLD}${CYAN}  Cretas V1 E2E — ${SUITE_NAME}${RESET}"
echo -e "${BOLD}${CYAN}============================================================${RESET}"
...
if [[ -n "$GREP_FILTER" ]]; then
    step "3/4" "Running Playwright tests (@pr-gate only)..."
else
    step "3/4" "Running ALL Playwright tests (@pr-gate + @post-deploy)..."
fi
echo ""
...
PW_EXIT=0
npx playwright test $GREP_FILTER --reporter=list || PW_EXIT=$?
...
if [[ "$PW_EXIT" -eq 0 ]]; then
    echo -e "${GREEN}${BOLD}  RESULT: ALL TESTS PASSED (${SUITE_NAME})${RESET}"
else
    echo -e "${RED}${BOLD}  RESULT: TESTS FAILED (exit $PW_EXIT) — ${SUITE_NAME}${RESET}"
