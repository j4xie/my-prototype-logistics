...
--- AA8: 行业术语-供应链制造业 (6) ---
  ...
  I [QUERY   ] SHIPMENT_QUERY                 LLM          0.48 | FOB价格查询
...
--- AI1: 拼写错误-库存领域同音字 (5) ---
  ...
  V [QUERY   ] MATERIAL_BATCH_QUERY           LLM          0.67 | 猪肉存活量
...
--- AM1: 餐饮-写入操作 (5) ---
  ...
  V [WRITE   ] FORM_GENERATION                LLM          0.67 | 今天的食材采购单生成一下
...
--- AM3: 餐饮-经营诊断分析 (5) ---
  ...
  T [UNKNOWN ] REPORT_FINANCE_REVENUE         LLM          0.64 | 人均消费是多少
...
--- AO2: 安全-XSS注入模式 (5) ---
  X [UNMATCHED] ERROR                          ?            ? | <script>alert(1)</script>查库存
  X [UNMATCHED] ERROR                          ?            ? | 查看<img src=x onerror=alert(1)>订单
...
--- AR1: 方言-东北话深度 (5) ---
  ...
  T [UNKNOWN ] QUERY_TODAY_TASK_COUNT         LLM          0.68 | 唠唠今天车间出了多少活儿
  === AR1: intent=4/5, type=4/5
...
--- AX4: 摄像头启动与配置 (6) ---
  T [QUERY   ] OUT_OF_DOMAIN                  LLM          0.64 | 打开摄像头
...
--- E5: 查询-溯源/追溯 (4) ---
  ...
  I [QUERY   ] REPORT_DASHBOARD_OVERVIEW      LLM          0.60 | 溯源码查询
...
--- T6: 写入-删除取消扩展 (5) ---
  ...
  V [WRITE   ] QUALITY_CHECK_EXECUTE          LLM          0.60 | 作废这张质检单
...
--- W2: 边界-中英文混合 (4) ---
  ...
  V [WRITE   ] SHIPMENT_CREATE                LLM          0.64 | 帮我create一个order
...
======================================================================
Intent accuracy:  1215/1232 (99%)
Type accuracy:    1143/1232 (93%)

!!! 89 TYPE CONFUSIONS (cross-contamination) !!!
  ...
  [AR1] "唠唠今天车间出了多少活儿" expected=QUERY actual=UNKNOWN intent=QUERY_TODAY_TASK_COUNT (东北话-唠唠/出活)
...
--- 17 Intent Mismatches ---
  ...
  [AR1] [LLM] "唠唠今天车间出了多少活儿" -> QUERY_TODAY_TASK_COUNT, expected: PRODUCTION_STATUS_QUERY|PROCESSING_BATCH_LIST|REPORT_PRODUCTION|N/A (东北话-唠唠/出活)
  ...
  [AV2] [LLM] "安排小陈去3号线" -> USER_ROLE_ASSIGN, expected: TASK_ASSIGN_BY_NAME|TASK_ASSIGN_WORKER|PROCESSING_WORKER_ASSIGN|SCHEDULING_EXECUTE_FOR_DATE|N/A (分配-小陈)
  ...
  [E5] [LLM] "溯源码查询" -> REPORT_DASHBOARD_OVERVIEW, expected: TRACE_PUBLIC|TRACE_BATCH|REPORT_QUALITY (溯源码)

======================================================================
PHASE 2: Response Quality (21:42:24)
======================================================================
  ...
  ~ [QUERY  ] SHIPMENT_CREATE                | 今日订单     | WARN:status=NEED_CLARIFICATION, OK:has_reply, WARN:no_data, OK:relevant(1/1kw)
    reply: 用户提到'今天有新订单'，在出货物流的语境下，这通常意味着需要基于新订单执行发货操作，即创建发货单。虽然语句简短，但隐含了启动发货流程的意图，与'创建发货单'的描述（用于向客户发送产品）最匹配。
  ...
  V [QUERY  ] SCHEDULING_LIST                | 排班计划     | OK:status, OK:has_reply, OK:has_data, OK:relevant(1/1kw), OK:time(22s)
  ...
  V [WRITE  ] CLOCK_OUT                      | 签退打卡     | OK:slot_filling, OK:has_reply, OK:has_data, OK:relevant(1/1kw)
    reply: 签退成功！您今天提前435分钟下班。今日工作时长：1.7小时。
  ...
  ~ [WRITE  ] OUT_OF_DOMAIN                  | 打开摄像头    | WARN:status=NEED_CLARIFICATION, OK:has_reply, OK:relevant(1/1kw), OK:time(42s)
    reply: 用户指令'打开摄像头'属于硬件设备控制或通用多媒体操作。在提供的意图列表中，没有针对'摄像头控制'、'视频监控开启'或'设备硬件开关'的特定意图。虽然存在'MEDIA_PLAY'（播放媒体），但其描述
...
FINAL SUMMARY
======================================================================
Phase 1 - Intent Routing:  1215/1232 (99%)
Phase 1 - Type Separation: 1143/1232 (93%)
Phase 1 - Cross-contamination: 89 cases
...
Started at 21:45:00
======================================================================
  ... 50/1232 done (21:45:20)
  ... 100/1232 done (21:45:33)
  ... 150/1232 done (21:45:51)
  ... 200/1232 done (21:46:01)
  ... 250/1232 done (21:46:11)
  ... 300/1232 done (21:46:21)
  ... 350/1232 done (21:46:28)
  ... 400/1232 done (21:46:42)
  ... 450/1232 done (21:46:53)
  ... 500/1232 done (21:47:04)
  ... 550/1232 done (21:47:16)
  ... 600/1232 done (21:47:30)
  ... 650/1232 done (21:47:36)
  ... 700/1232 done (21:47:40)
  ... 750/1232 done (21:47:44)
  ... 800/1232 done (21:47:48)
  ... 850/1232 done (21:47:52)
  ... 900/1232 done (21:47:56)
  ... 950/1232 done (21:48:02)
  ... 1000/1232 done (21:48:05)
  ... 1050/1232 done (21:48:10)
  ... 1100/1232 done (21:48:16)
  ... 1150/1232 done (21:48:23)
  ... 1200/1232 done (21:48:30)
  Completed all 1232 at 21:48:43

--- Per-Category Quality ---
  A1: 8/8 PASS, 0 WARN, 0 FAIL (100%) [咨询-食品安全基础]
  A2: 7/8 PASS, 0 WARN, 1 FAIL (88%) [咨询-食品安全/检测] <<< FAIL
  A3: 7/8 PASS, 0 WARN, 1 FAIL (88%) [咨询-生产工艺知识] <<< FAIL
...
--- FAIL Cases (10) ---
  [A2] "食品过敏原标识要求" → ERROR | status=ERROR | API_ERROR, STATUS_ERROR, NO_REPLY
    reply: (empty)
  [A3] "肉制品加工卫生要求" → ERROR | status=ERROR | API_ERROR, STATUS_ERROR, NO_REPLY
    reply: (empty)
...
======================================================================
Total: 1232 | PASS: 1222 (99%) | WARN: 0 | FAIL: 10
Acceptable (PASS+WARN): 1222/1232 (99%)
Categories needing attention: A2, A3, AG2, AO3, AU2, U4, Z4
...
Phase 2b — Full Quality Scan:   1222/1232 full-pass (99%), 1222/1232 acceptable (99%)
