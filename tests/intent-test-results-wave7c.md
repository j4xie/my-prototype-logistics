...
--- AB10: 用户管理-禁用分配角色 (6) ---
  V [WRITE   ] USER_DISABLE                   CLASSIFIER   0.81 | 禁用这个用户账号
...
--- AB3: 反问句-难道反问修辞 (5) ---
  ...
  I [QUERY   ] RESTAURANT_WASTAGE_SUMMARY     SEMANTIC     1.00 | 连基本的产量都不达标吗
...
--- AB5: 语气词-嘛啦呗咯句末 (6) ---
  ...
  V [WRITE   ] CLOCK_IN                       CLASSIFIER   0.98 | 帮我打个卡啦
...
--- AB7: 省略-同上一样继续 (5) ---
  ...
  V [QUERY   ] CONTEXT_CONTINUE               SEMANTIC     0.95 | 一样的，再查一遍
...
--- AC1: 餐饮-菜品查询 (6) ---
  ...
  V [QUERY   ] COST_TREND_ANALYSIS            CLASSIFIER   0.96 | 做红烧肉的成本分析
...
--- AH11: 对抗-语境切换中断 (5) ---
  I [QUERY   ] SCHEDULING_LIST                SEMANTIC     0.98 | 刚才的那个忘了帮我查一下考勤
...
--- AH12: 角色-车间主管视角 (5) ---
  T [UNKNOWN ] SCALE_CALIBRATE                SEMANTIC     1.00 | 我手下几个工人在干活
...
--- AH2: 发货-按日期/更新 (5) ---
  ...
  V [WRITE   ] SHIPMENT_UPDATE                CLASSIFIER   0.99 | 更新这条发货单信息
...
--- AH7: 系统-通知配置 (4) ---
  ...
  V [WRITE   ] FACTORY_NOTIFICATION_CONFIG    CLASSIFIER   0.99 | 修改工厂通知配置
...
--- AI4: 拼写错误-发货订单HR (5) ---
  ...
  I [QUERY   ] SOP_ANALYZE_COMPLEXITY         SEMANTIC     0.98 | 考勤已常记录
  X [UNMATCHED] N/A                            ?            ? | 帮我打咔
  V [QUERY   ] SCHEDULING_LIST                PHRASE_MATCH 0.98 | 排版表
  === AI4: intent=4/5, type=5/5
...
--- AN2: 多轮-维度切换追问 (5) ---
  ...
  === AN2: intent=3/5, type=4/5

--- AN3: 多轮-确认/否定/修正上文 (5) ---
  V [QUERY   ] CONTEXT_CONTINUE               SEMANTIC     0.98 | 对就是这个再查详细一点
...
--- AP3: 数字-多数值组合查询 (5) ---
  ...
  V [QUERY   ] QUALITY_CHECK_QUERY            CLASSIFIER   0.98 | 查看批次号B001到B010的质检
...
--- AX5: 流水账混合多意图句 (4) ---
  ...
  I [QUERY   ] REPORT_FINANCE                 SEMANTIC     0.99 | 看看今天产量顺便把质检报告拉出来再安排明天排班
  V [QUERY   ] ATTENDANCE_STATUS              PHRASE_MATCH 0.98 | 张三打卡了吗如果没来就安排李四顶班然后告诉车间主管
  I [QUERY   ] QUALITY_CHECK_QUERY            SEMANTIC     0.97 | 检查设备状态把坏的报修同时催一下维修进度
  === AX5: intent=2/4, type=4/4
...
--- C2: 写入-状态更新/打卡 (4) ---
  ...
  V [WRITE   ] CLOCK_IN                       CLASSIFIER   0.98 | 我要签到
...
--- C3: 写入-更多动词模式 (6) ---
  ...
  V [WRITE   ] CLOCK_IN                       CLASSIFIER   0.98 | 签到打卡
...
--- D2: 边界-查询vs写入 (8) ---
  ...
  V [WRITE   ] CLOCK_IN                       CLASSIFIER   0.98 | 我要打卡
...
--- D5: 边界-查询vs写入深层混淆 (6) ---
  ...
  V [WRITE   ] SHIPMENT_STATUS_UPDATE         SEMANTIC     0.85 | 确认发货
...
--- H5: 查询-库存深层 (6) ---
  ...
  V [QUERY   ] MATERIAL_EXPIRING_ALERT        CLASSIFIER   0.98 | 即将过期的原材料
...
--- U1: 查询-设备分析诊断 (5) ---
  ...
  V [QUERY   ] EQUIPMENT_STATUS_QUERY         PHRASE_MATCH 0.98 | 设备健康诊断
  V [QUERY   ] EQUIPMENT_BREAKDOWN_REPORT     PHRASE_MATCH 0.98 | 设备故障报告
  X [UNMATCHED] ERROR                          ?            ? | 按名称查设备状态
  X [UNMATCHED] ERROR                          ?            ? | 今天车间的日报
  === U1: intent=3/5, type=3/5

--- U2: 写入-设备操作扩展 (4) ---
  X [UNMATCHED] ERROR                          ?            ? | 启动摄像头
  X [UNMATCHED] ERROR                          ?            ? | 打开监控
  X [UNMATCHED] ERROR                          ?            ? | 解除设备告警
  X [UNMATCHED] ERROR                          ?            ? | CCP监控点数据检测
  === U2: intent=0/4, type=0/4

--- U3: 查询-生产过程详情 (5) ---
  X [UNMATCHED] ERROR                          ?            ? | 这个批次有哪些工人在做
  X [UNMATCHED] ERROR                          ?            ? | 这个批次的负责人是谁
  X [UNMATCHED] ERROR                          ?            ? | 当前生产到哪一步了
  X [UNMATCHED] ERROR                          ?            ? | 查看生产工序
  X [UNMATCHED] ERROR                          ?            ? | 现在车间有多少人在
  === U3: intent=0/5, type=0/5

--- U4: 写入-工人管理操作 (4) ---
  X [UNMATCHED] ERROR                          ?            ? | 确认工人到岗
  X [UNMATCHED] ERROR                          ?            ? | 确认生产人员已就位
  X [UNMATCHED] ERROR                          ?            ? | 工人签退下线
  X [UNMATCHED] ERROR                          ?            ? | 把李四安排到包装岗位
  === U4: intent=1/4, type=1/4

--- U5: 查询-审批/待办/物料 (5) ---
  X [UNMATCHED] ERROR                          ?            ? | 查看审批记录
  X [UNMATCHED] ERROR                          ?            ? | 订单还缺多少原料
  X [UNMATCHED] ERROR                          ?            ? | 这批猪肉退货原因
  X [UNMATCHED] ERROR                          ?            ? | 运输线路查询
  X [UNMATCHED] ERROR                          ?            ? | 我的待办事项
  === U5: intent=0/5, type=0/5

--- U6: 查询-AI质检报告 (4) ---
  X [UNMATCHED] ERROR                          ?            ? | AI质检分析报告
  X [UNMATCHED] ERROR                          ?            ? | 智能质量分析
  X [UNMATCHED] ERROR                          ?            ? | 质检审核报告
  X [UNMATCHED] ERROR                          ?            ? | 利润趋势分析
  === U6: intent=0/4, type=0/4

--- V1: 写入-出库发货扩展 (4) ---
  X [UNMATCHED] ERROR                          ?            ? | 出库一批猪肉
...
--- W3: 边界-否定句式 (5) ---
  ...
  V [QUERY   ] REPORT_PRODUCTION              CLASSIFIER   0.99 | 别查生产，看看设备
...
--- Z1: 上下文-代词回指 (5) ---
  ...
  V [QUERY   ] SUPPLIER_SEARCH                SEMANTIC     1.00 | 再查一下那个供应商
...
--- Z3: 代码混用-行业缩写 (6) ---
  ...
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | ERP里的库存数据
...
======================================================================
Intent accuracy:  1176/1232 (95%)
Type accuracy:    1126/1232 (91%)

!!! 82 TYPE CONFUSIONS (cross-contamination) !!!
  ...
  [AH12] "我手下几个工人在干活" expected=QUERY actual=UNKNOWN intent=SCALE_CALIBRATE (主管-车间人数)
...
--- 56 Intent Mismatches ---
  ...
  [AB3] [SEMANTIC] "连基本的产量都不达标吗" -> RESTAURANT_WASTAGE_SUMMARY, expected: REPORT_PRODUCTION|PROCESSING_BATCH_LIST|PRODUCTION_STATUS_QUERY|N/A (连+产量)
  ...
  [AH11] [SEMANTIC] "刚才的那个忘了帮我查一下考勤" -> SCHEDULING_LIST, expected: ATTENDANCE_HISTORY|ATTENDANCE_STATS|ATTENDANCE_TODAY|CONTEXT_CONTINUE|N/A (放弃上文+新意图)
  [AH12] [SEMANTIC] "我手下几个工人在干活" -> SCALE_CALIBRATE, expected: WORKER_IN_SHOP_REALTIME_COUNT|PROCESSING_BATCH_WORKERS|ATTENDANCE_TODAY|N/A (主管-车间人数)
  ...
  [AI4] [SEMANTIC] "考勤已常记录" -> SOP_ANALYZE_COMPLEXITY, expected: ATTENDANCE_ANOMALY|ATTENDANCE_STATS|ATTENDANCE_HISTORY|N/A (异常→已常(同音typo))
  ...
  [AS2] [SEMANTIC] "客户催了三次了赶紧查发货" -> CUSTOMER_SEARCH, expected: SHIPMENT_QUERY|ORDER_STATUS|SHIPMENT_EXPEDITE|SHIPMENT_BY_CUSTOMER|N/A (恐慌-客户催促)
  [AS3] [SEMANTIC] "都第三次问了请问订单到底发了没" -> PROCESSING_BATCH_LIST, expected: ORDER_STATUS|SHIPMENT_QUERY|ORDER_LIST|N/A (阴阳-第三次问)
  ...
  [AX5] [SEMANTIC] "看看今天产量顺便把质检报告拉出来再安排明天排班" -> REPORT_FINANCE, expected: PRODUCTION_STATUS_QUERY|REPORT_PRODUCTION|QUALITY_CHECK_QUERY|SCHEDULING_EXECUTE_FOR_DATE|N/A (三意图-产量+质检+排班)
  [AX5] [SEMANTIC] "检查设备状态把坏的报修同时催一下维修进度" -> QUALITY_CHECK_QUERY, expected: EQUIPMENT_STATUS_QUERY|EQUIPMENT_STATUS_UPDATE|ALERT_DIAGNOSE|SHIPMENT_EXPEDITE|N/A (三意图-设备+报修+催进度)
  ...
  [U1] [?] "按名称查设备状态" -> ERROR, expected: QUERY_EQUIPMENT_STATUS_BY_NAME|EQUIPMENT_STATUS_QUERY|EQUIPMENT_DETAIL (按名称查设备)
  [U1] [?] "今天车间的日报" -> ERROR, expected: REPORT_WORKSHOP_DAILY|REPORT_PRODUCTION|REPORT_EXECUTIVE_DAILY (车间日报)
  [U2] [?] "启动摄像头" -> ERROR, expected: EQUIPMENT_CAMERA_START|EQUIPMENT_START|OPEN_CAMERA (启动摄像头)
  [U2] [?] "打开监控" -> ERROR, expected: EQUIPMENT_CAMERA_START|OPEN_CAMERA|EQUIPMENT_START (打开监控)
  [U2] [?] "解除设备告警" -> ERROR, expected: EQUIPMENT_ALERT_RESOLVE|ALERT_RESOLVE|EQUIPMENT_ALERT_ACKNOWLEDGE (解除告警)
  [U2] [?] "CCP监控点数据检测" -> ERROR, expected: CCP_MONITOR_DATA_DETECTION|QUALITY_CHECK_QUERY|QUALITY_STATS (CCP监测)
  [U3] [?] "这个批次有哪些工人在做" -> ERROR, expected: PROCESSING_BATCH_WORKERS|PROCESSING_BATCH_DETAIL|PROCESSING_WORKER_ASSIGN|PROCESSING_BATCH_LIST (批次工人)
  [U3] [?] "这个批次的负责人是谁" -> ERROR, expected: QUERY_PROCESSING_BATCH_SUPERVISOR|PROCESSING_BATCH_DETAIL|TASK_PROGRESS_QUERY (批次负责人)
  [U3] [?] "当前生产到哪一步了" -> ERROR, expected: QUERY_PROCESSING_CURRENT_STEP|QUERY_PROCESSING_STEP|PROCESSING_BATCH_DETAIL (当前步骤)
  [U3] [?] "查看生产工序" -> ERROR, expected: QUERY_PROCESSING_STEP|PROCESSING_BATCH_DETAIL|PROCESSING_BATCH_LIST (生产工序)
  [U3] [?] "现在车间有多少人在" -> ERROR, expected: WORKER_IN_SHOP_REALTIME_COUNT|QUERY_ONLINE_STAFF_COUNT|ATTENDANCE_TODAY (车间人数)
  [U4] [?] "确认工人到岗" -> ERROR, expected: WORKER_ARRIVAL_CONFIRM|PRODUCTION_CONFIRM_WORKERS_PRESENT|CLOCK_IN (确认到岗)
  [U4] [?] "确认生产人员已就位" -> ERROR, expected: PRODUCTION_CONFIRM_WORKERS_PRESENT|WORKER_ARRIVAL_CONFIRM|PROCESSING_BATCH_START (确认就位)
  [U4] [?] "工人签退下线" -> ERROR, expected: PROCESSING_WORKER_CHECKOUT|CLOCK_OUT|PROCESSING_BATCH_COMPLETE (签退下线)
  [U5] [?] "查看审批记录" -> ERROR, expected: QUERY_APPROVAL_RECORD|ORDER_APPROVAL|ORDER_LIST (审批记录)
  [U5] [?] "订单还缺多少原料" -> ERROR, expected: QUERY_ORDER_PENDING_MATERIAL_QUANTITY|MATERIAL_BATCH_QUERY|MATERIAL_LOW_STOCK_ALERT (待补物料)
  [U5] [?] "这批猪肉退货原因" -> ERROR, expected: QUERY_MATERIAL_REJECTION_REASON|QUALITY_CHECK_QUERY|MATERIAL_BATCH_QUERY (退货原因)
  [U5] [?] "运输线路查询" -> ERROR, expected: QUERY_TRANSPORT_LINE|SHIPMENT_QUERY|SHIPMENT_QUERY (运输线路)
  [U5] [?] "我的待办事项" -> ERROR, expected: USER_TODO_LIST|TASK_PROGRESS_QUERY|ORDER_LIST (待办事项)
  [U6] [?] "AI质检分析报告" -> ERROR, expected: REPORT_AI_QUALITY|REPORT_INTELLIGENT_QUALITY|REPORT_QUALITY_AI|REPORT_QUALITY (AI质检)
  [U6] [?] "智能质量分析" -> ERROR, expected: REPORT_INTELLIGENT_QUALITY|REPORT_AI_QUALITY|REPORT_QUALITY|QUALITY_STATS (智能质量)
  [U6] [?] "质检审核报告" -> ERROR, expected: REPORT_CHECK|QUALITY_CHECK_QUERY|QUALITY_CHECK_QUERY (质检审核)
  [U6] [?] "利润趋势分析" -> ERROR, expected: PROFIT_TREND_ANALYSIS|REPORT_TRENDS|REPORT_FINANCE|COST_TREND_ANALYSIS (利润趋势)
  [V1] [?] "出库一批猪肉" -> ERROR, expected: INVENTORY_OUTBOUND|WAREHOUSE_OUTBOUND|MATERIAL_BATCH_CONSUME|SHIPMENT_CREATE (出库)
...
======================================================================
PHASE 2: Response Quality (16:46:58)
======================================================================
  ...
  V [WRITE  ] CLOCK_OUT                      | 签退打卡     | OK:slot_filling, OK:has_reply, OK:has_data, OK:relevant(1/1kw)
    reply: 签退成功！您今天提前731分钟下班。今日工作时长：-3.2小时。
...
--- Phase 2 Summary ---
Total: 94 | PASS: 79 | WARN: 15 | FAIL: 0
Pass rate: 79/94 (84%) full-pass
Acceptable: 94/94 (100%) (pass+warn)

--- Handler Quality Issues (13 handlers) ---
...
FINAL SUMMARY
======================================================================
Phase 1 - Intent Routing:  1176/1232 (95%)
Phase 1 - Type Separation: 1126/1232 (91%)
Phase 1 - Cross-contamination: 82 cases
Phase 2 - Response Quality: 79/94 full-pass, 94/94 acceptable
Phase 2 - Handler Issues: 13 handlers with quality problems
...
Started at 16:48:16
======================================================================
  ... 50/1232 done (16:48:23)
  ... 100/1232 done (16:48:26)
  ... 150/1232 done (16:48:29)
  ... 200/1232 done (16:48:35)
  ... 250/1232 done (16:48:39)
  ... 300/1232 done (16:48:43)
  ... 350/1232 done (16:48:46)
  ... 400/1232 done (16:48:52)
  ... 450/1232 done (16:48:55)
  ... 500/1232 done (16:48:58)
  ... 550/1232 done (16:49:05)
  ... 600/1232 done (16:49:12)
  ... 650/1232 done (16:49:19)
  ... 700/1232 done (16:49:25)
  ... 750/1232 done (16:49:31)
  ... 800/1232 done (16:49:37)
  ... 850/1232 done (16:49:42)
  ... 900/1232 done (16:49:47)
  ... 950/1232 done (16:49:52)
  ... 1000/1232 done (16:49:56)
  ... 1050/1232 done (16:50:00)
  ... 1100/1232 done (16:50:04)
  ... 1150/1232 done (16:50:10)
  ... 1200/1232 done (16:50:15)
  Completed all 1232 at 16:50:24
...
--- Per-Category Quality ---
  ...
  AI4: 4/5 PASS, 1 WARN, 0 FAIL (80%) [拼写错误-发货订单HR]
...
--- WARN Cases (1) ---
  [AI4] "考勤已常记录" → FOOD_KNOWLEDGE_QUERY | SLOW(64s) | reply_len=423

--- Issue Distribution ---
  SLOW: 1 cases

--- Worst Intents (by quality issues) ---
  FOOD_KNOWLEDGE_QUERY: 62/63 PASS (98%), 0 FAIL, 1 WARN

======================================================================
PHASE 2b SUMMARY: Full Quality Scan
======================================================================
Total: 1232 | PASS: 1231 (100%) | WARN: 1 | FAIL: 0
Acceptable (PASS+WARN): 1232/1232 (100%)
...
======================================================================
Phase 1  — Intent Routing:      1176/1232 (95%)
Phase 2  — Curated Quality:     79/94 full-pass, 94/94 acceptable
Phase 2b — Full Quality Scan:   1231/1232 full-pass (100%), 1232/1232 acceptable (100%)
