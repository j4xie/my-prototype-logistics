// Serial retest v3 — fixes v2's OOM cascade.
// Strategy: 8s gap between requests + skip 2 known OOM triggers + 90s timeout.
// 9MB 唏嘛香 + 4.3MB 桂满陇 csv tested SEPARATELY isolated (not here).
...
// 21 files NOT yet verified (we got OK on first 11 of v2 retest before OOM cascade).
// These were FAIL in v4 parallel but are mostly small — should be OK.
const UNTESTED = [
...
// Files that hit circuit breaker on v2 retest after OOM — re-verify
const CB_VICTIMS = [
...
// Mojibake .xls files — Node fs needs literal mojibake names
const MOJIBAKE = [
...
const ALL_FILES = [...CB_VICTIMS, ...UNTESTED, ...MOJIBAKE];
const GAP_MS = 8000;  // 8s gap between requests for Python memory recovery
...
function fetch(url, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    req.on('timeout', () => { req.destroy(new Error(`upload timeout ${TIMEOUT_MS/1000}s`)); });
...
async function main() {
  console.log(`v3 retest — ${ALL_FILES.length} files, 8s gap, 90s timeout, skip OOM triggers\n`);
  const token = await login();
  console.log(`Logged in (${token.slice(0, 20)}...)\n`);

  const results = [];
  for (let i = 0; i < ALL_FILES.length; i++) {
    const rel = ALL_FILES[i];
    const fp = path.join(ROOT, rel);
    if (!fs.existsSync(fp)) {
      console.log(`[${i+1}/${ALL_FILES.length}] ${rel}: FILE_NOT_FOUND_BY_NODE`);
    ...
    try {
      ...
      entry = { rel, status, httpStatus: r.httpStatus, fileSize: r.fileSize,
        rows: detectedRows, cols: detectedCols, tableType, elapsed, errMsg };
      console.log(`[${i+1}/${ALL_FILES.length}] ${rel}: ${status} (${(r.fileSize/1024).toFixed(0)}KB, ${elapsed}s${detectedRows ? `, ${detectedRows}行×${detectedCols}列` : ''}${errMsg ? `, err=${errMsg.slice(0,80)}` : ''})`);
    } catch (e) {
      const elapsed = ((Date.now() - start) / 1000).toFixed(1);
      entry = { rel, status: 'EXCEPTION', error: e.message, elapsed };
      console.log(`[${i+1}/${ALL_FILES.length}] ${rel}: EXCEPTION (${elapsed}s): ${e.message}`);
    }
    results.push(entry);
    if (i < ALL_FILES.length - 1) await sleep(GAP_MS);
  }
  ...
  if (fail.length) {
    ...
    fail.forEach(r => console.log(`  ${r.rel} (${(r.fileSize/1024).toFixed(0)}KB): ${r.errMsg ?? '-'}`));
  }
  if (exc.length) {
    console.log('\n=== EXCEPTION/NOT_FOUND ===');
  ...
  const outPath = path.join(ROOT, '..', '..', '..', 'tests/e2e-comprehensive/results/bulk-audit-retest-v3.json');
