...
--- AA5: 纠错-自我修正表达 (6) ---
  V [QUERY   ] ORDER_LIST                     CLASSIFIER   0.98 | 不对，我要的是库存不是订单
...
--- AB14: 嵌入-URL电话特殊字符 (6) ---
  ...
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 批次#B20240115的库存
...
--- AB2: 话题句-话题述题结构 (6) ---
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 库存嘛，查一下
...
--- AB3: 反问句-难道反问修辞 (5) ---
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 难道猪肉库存真的没了？
...
--- AC2: 餐饮-食材库存 (6) ---
  ...
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 低库存的食材有哪些
...
--- AE2: 秤-故障排查与校准 (5) ---
  V [QUERY   ] REPORT_DASHBOARD_OVERVIEW      CLASSIFIER   0.97 | 电子秤数据不准帮我排查
...
--- AH11: 对抗-语境切换中断 (5) ---
  ...
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 先不说那个库存怎么样
...
--- AH5: 库存-清零操作 (4) ---
  ...
  T [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 把库存全部出清
...
--- AK2: 特殊字符-符号夹杂业务查询 (5) ---
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 【紧急】查一下库存
...
--- AO1: 安全-SQL注入模式 (5) ---
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 查询库存'; DROP TABLE materials;--
...
--- AO2: 安全-XSS注入模式 (5) ---
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | <script>alert(1)</script>查库存
...
--- AP3: 数字-多数值组合查询 (5) ---
  ...
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 5号冷库3层A区的猪肉库存
...
--- AS2: 情绪-紧急恐慌 (5) ---
  ...
  V [QUERY   ] SHIPMENT_QUERY                 CLASSIFIER   0.99 | 客户催了三次了赶紧查发货
...
--- D5: 边界-查询vs写入深层混淆 (6) ---
  ...
  V [QUERY   ] MATERIAL_BATCH_QUERY           CLASSIFIER   0.88 | 查看库存不足的原材料
...
--- T4: 对抗-跨域连词bypass (3) ---
  I [QUERY   ] REPORT_INVENTORY               SEMANTIC     1.00 | 库存不够顺便查一下排班
...
--- V1: 写入-出库发货扩展 (4) ---
  ...
  X [UNMATCHED] ERROR                          ?            ? | 仓库出库操作
  X [UNMATCHED] ERROR                          ?            ? | 通知仓库备货
  X [UNMATCHED] ERROR                          ?            ? | MRP物料需求计算
  === V1: intent=1/4, type=1/4

--- V2: 写入-排班计划扩展 (4) ---
  X [UNMATCHED] ERROR                          ?            ? | 安排明天的排班
  X [UNMATCHED] ERROR                          ?            ? | 执行2月25号的排班
  X [UNMATCHED] ERROR                          ?            ? | 配置采购审批流程
  X [UNMATCHED] ERROR                          ?            ? | 查看排班覆盖情况
  === V2: intent=1/4, type=1/4
...
======================================================================
Intent accuracy:  1193/1232 (97%)
Type accuracy:    1131/1232 (92%)

!!! 86 TYPE CONFUSIONS (cross-contamination) !!!
...
--- 39 Intent Mismatches ---
  ...
  [T4] [SEMANTIC] "库存不够顺便查一下排班" -> REPORT_INVENTORY, expected: MATERIAL_BATCH_QUERY|MATERIAL_LOW_STOCK_ALERT|SCHEDULING_LIST|N/A (跨域可UNMATCHED)
  ...
  [V1] [?] "仓库出库操作" -> ERROR, expected: WAREHOUSE_OUTBOUND|INVENTORY_OUTBOUND|MATERIAL_BATCH_CONSUME (仓库出库)
  [V1] [?] "通知仓库备货" -> ERROR, expected: SHIPMENT_NOTIFY_WAREHOUSE_PREPARE|SHIPMENT_CREATE|SHIPMENT_EXPEDITE (通知备货)
  [V2] [?] "安排明天的排班" -> ERROR, expected: SCHEDULING_EXECUTE_FOR_DATE|SCHEDULING_RUN_TOMORROW|SCHEDULING_SET_AUTO (排班执行)
  [V2] [?] "执行2月25号的排班" -> ERROR, expected: SCHEDULING_EXECUTE_FOR_DATE|SCHEDULING_RUN_TOMORROW|SCHEDULING_SET_AUTO (指定日期排班)
  [V2] [?] "查看排班覆盖情况" -> ERROR, expected: SCHEDULING_LIST|SCHEDULING_LIST (排班覆盖)
...
======================================================================
PHASE 2: Response Quality (17:55:17)
======================================================================
  ...
  V [WRITE  ] CLOCK_OUT                      | 签退打卡     | OK:slot_filling, OK:has_reply, OK:has_data, OK:relevant(1/1kw)
    reply: 签退成功！您今天提前663分钟下班。今日工作时长：-2.1小时。
...
FINAL SUMMARY
======================================================================
Phase 1 - Intent Routing:  1193/1232 (97%)
Phase 1 - Type Separation: 1131/1232 (92%)
Phase 1 - Cross-contamination: 86 cases
...
Started at 17:56:32
======================================================================
  ... 50/1232 done (17:56:40)
  ... 100/1232 done (17:56:44)
  ... 150/1232 done (17:56:47)
  ... 200/1232 done (17:56:52)
  ... 250/1232 done (17:56:56)
  ... 300/1232 done (17:57:00)
  ... 350/1232 done (17:57:04)
  ... 400/1232 done (17:57:09)
  ... 450/1232 done (17:57:12)
  ... 500/1232 done (17:57:15)
  ... 550/1232 done (17:57:20)
  ... 600/1232 done (17:57:27)
  ... 650/1232 done (17:57:31)
  ... 700/1232 done (17:57:35)
  ... 750/1232 done (17:57:39)
  ... 800/1232 done (17:57:43)
  ... 850/1232 done (17:57:47)
  ... 900/1232 done (17:57:52)
  ... 950/1232 done (17:57:57)
  ... 1000/1232 done (17:58:01)
  ... 1050/1232 done (17:58:05)
  ... 1100/1232 done (17:58:08)
  ... 1150/1232 done (17:58:13)
  ... 1200/1232 done (17:58:17)
  Completed all 1232 at 17:58:24
...
======================================================================
Total: 1232 | PASS: 1232 (100%) | WARN: 0 | FAIL: 0
Acceptable (PASS+WARN): 1232/1232 (100%)
Unique issue types: 0
...
Phase 2b — Full Quality Scan:   1232/1232 full-pass (100%), 1232/1232 acceptable (100%)
