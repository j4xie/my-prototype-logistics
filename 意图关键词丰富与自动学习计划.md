...
关键词晋升机制（任务5）实现后，当关键词从3+工厂晋升到全局(GLOBAL)时，数据正确存储到 `keyword_effectiveness` 表，但意图匹配时无法使用这些全局关键词。

### 根因分析

`AIIntentServiceImpl.java` 的匹配方法只读取 `AIIntentConfig.keywords` JSON 字段，不读取 `keyword_effectiveness` 表：

```java
// 问题代码 - 只读取 JSON 字段
List<String> keywords = parseKeywordsFromJson(intent.getKeywords());
```

### 修复方案

新增 `getAllKeywordsForMatching()` 方法，合并三层关键词来源：

1. **基础关键词** - `AIIntentConfig.keywords` JSON 字段
2. **工厂级关键词** - `keyword_effectiveness` 表，当前工厂，effectiveness >= 0.5
3. **全局关键词** - `keyword_effectiveness` 表，factoryId = "GLOBAL"

### 修改文件

- `backend-java/src/main/java/com/cretas/aims/service/impl/AIIntentServiceImpl.java`
  - 新增: `getAllKeywordsForMatching(factoryId, intent)`
  - 修改: `getMatchedKeywords()` 接受 factoryId 参数
  - 修改: `calculateKeywordMatchScore()` 接受 factoryId 参数
