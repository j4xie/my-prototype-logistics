...
CREATE TABLE IF NOT EXISTS decoration_image_library (
    ...
    industry_type VARCHAR(50) COMMENT '行业类型: food/gift/seafood/dessert/baby/tech/pet/tea/art/business',
    style_type VARCHAR(50) COMMENT '风格类型: fresh/luxury/warm/minimal/festive/modern',
    theme_code VARCHAR(50) COMMENT '关联主题代码',
    width INT DEFAULT 750 COMMENT '图片宽度',
    height INT DEFAULT 300 COMMENT '图片高度',
    ...
    INDEX idx_industry_style (industry_type, style_type)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='装修图片素材库';
...
-- 创建主题与图片关联表
-- ============================================
CREATE TABLE IF NOT EXISTS decoration_theme_image_rel (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    theme_id BIGINT NOT NULL COMMENT '主题ID',
    image_id BIGINT NOT NULL COMMENT '图片ID',
    position VARCHAR(50) COMMENT '图片位置: header/banner1/banner2/background/footer',
    is_default TINYINT DEFAULT 0 COMMENT '是否默认图片',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uk_theme_image_position (theme_id, image_id, position),
    INDEX idx_theme_id (theme_id),
    INDEX idx_image_id (image_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='主题图片关联表';
