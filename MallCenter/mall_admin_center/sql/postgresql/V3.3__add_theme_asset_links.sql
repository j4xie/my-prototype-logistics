-- =============================================
-- V3.3: 为主题预设添加素材关联字段 - PostgreSQL 版本
...
ADD COLUMN IF NOT EXISTS recommended_images TEXT,
ADD COLUMN IF NOT EXISTS recommended_component_styles TEXT,
ADD COLUMN IF NOT EXISTS recommended_layout_id BIGINT,
ADD COLUMN IF NOT EXISTS recommended_icons TEXT,
ADD COLUMN IF NOT EXISTS recommended_fonts TEXT;

COMMENT ON COLUMN decoration_theme_preset.recommended_images IS '推荐图片ID列表JSON';
COMMENT ON COLUMN decoration_theme_preset.recommended_component_styles IS '推荐组件样式JSON';
COMMENT ON COLUMN decoration_theme_preset.recommended_layout_id IS '推荐布局预设ID';
COMMENT ON COLUMN decoration_theme_preset.recommended_icons IS '推荐图标JSON';
COMMENT ON COLUMN decoration_theme_preset.recommended_fonts IS '推荐字体样式JSON';

-- 2. 创建布局预设表（如果不存在）
DROP TABLE IF EXISTS decoration_layout_preset CASCADE;
CREATE TABLE decoration_layout_preset (
...
COMMENT ON TABLE decoration_layout_preset IS '布局预设表';
COMMENT ON COLUMN decoration_layout_preset.name IS '布局名称';
COMMENT ON COLUMN decoration_layout_preset.code IS '布局编码';
COMMENT ON COLUMN decoration_layout_preset.description IS '布局描述';
COMMENT ON COLUMN decoration_layout_preset.preview_image IS '预览图URL';
COMMENT ON COLUMN decoration_layout_preset.layout_config IS '布局配置JSON';
COMMENT ON COLUMN decoration_layout_preset.industry_tags IS '适用行业标签';
COMMENT ON COLUMN decoration_layout_preset.status IS '状态：0禁用 1启用';
COMMENT ON COLUMN decoration_layout_preset.sort_order IS '排序';

-- Trigger for update_time auto-update
CREATE OR REPLACE FUNCTION update_decoration_layout_preset_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_decoration_layout_preset_update_time
    BEFORE UPDATE ON decoration_layout_preset
    FOR EACH ROW
    EXECUTE FUNCTION update_decoration_layout_preset_updated_at();
