-- ============================================
-- V3.5 装修Prompt模板和关键词映射表 - PostgreSQL 版本
-- Phase 8: AI引导式对话装修系统
-- Converted from MySQL
-- ============================================

-- 1. Prompt模板表
DROP TABLE IF EXISTS decoration_prompt_template CASCADE;
CREATE TABLE decoration_prompt_template (
    id BIGSERIAL PRIMARY KEY,
    name VARCHAR(100) NOT NULL,
    code VARCHAR(50) NOT NULL UNIQUE,
    industry_type VARCHAR(30) NOT NULL,
    image_type VARCHAR(20) NOT NULL,
    style_type VARCHAR(30) DEFAULT 'general',
    base_prompt TEXT NOT NULL,
    variables_def TEXT,
    negative_prompt VARCHAR(500),
    recommended_size VARCHAR(20) DEFAULT '1280*720',
...
CREATE INDEX idx_prompt_industry_type ON decoration_prompt_template(industry_type);
CREATE INDEX idx_prompt_image_type ON decoration_prompt_template(image_type);

COMMENT ON TABLE decoration_prompt_template IS '装修Prompt模板表';
COMMENT ON COLUMN decoration_prompt_template.id IS '主键ID';
COMMENT ON COLUMN decoration_prompt_template.name IS '模板名称';
COMMENT ON COLUMN decoration_prompt_template.code IS '模板编码（唯一）';
COMMENT ON COLUMN decoration_prompt_template.industry_type IS '行业类型: fresh_food/seafood/dessert/gift/baby/tech/beauty/general';
COMMENT ON COLUMN decoration_prompt_template.image_type IS '图片类型: banner/background/icon/product';
COMMENT ON COLUMN decoration_prompt_template.style_type IS '风格类型: fresh/luxury/minimal/dopamine/warm';
COMMENT ON COLUMN decoration_prompt_template.base_prompt IS '基础prompt模板，支持变量: {product}, {style}, {color_tone}, {size}';
COMMENT ON COLUMN decoration_prompt_template.variables_def IS '变量定义JSON';
COMMENT ON COLUMN decoration_prompt_template.negative_prompt IS '负向提示词';
COMMENT ON COLUMN decoration_prompt_template.recommended_size IS '推荐尺寸';
COMMENT ON COLUMN decoration_prompt_template.example_image IS '示例图片URL';
COMMENT ON COLUMN decoration_prompt_template.use_count IS '使用次数';
COMMENT ON COLUMN decoration_prompt_template.status IS '状态：0禁用 1启用';
COMMENT ON COLUMN decoration_prompt_template.sort_order IS '排序';

-- Trigger for update_time auto-update
CREATE OR REPLACE FUNCTION update_decoration_prompt_template_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_decoration_prompt_template_update_time
    BEFORE UPDATE ON decoration_prompt_template
    FOR EACH ROW
    EXECUTE FUNCTION update_decoration_prompt_template_updated_at();

-- 2. 关键词映射表
DROP TABLE IF EXISTS decoration_keyword_mapping CASCADE;
CREATE TABLE decoration_keyword_mapping (
    id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX idx_keyword ON decoration_keyword_mapping(keyword);
CREATE INDEX idx_mapping_type ON decoration_keyword_mapping(mapping_type);

COMMENT ON TABLE decoration_keyword_mapping IS '装修关键词映射表';
COMMENT ON COLUMN decoration_keyword_mapping.id IS '主键ID';
COMMENT ON COLUMN decoration_keyword_mapping.keyword IS '关键词';
COMMENT ON COLUMN decoration_keyword_mapping.mapping_type IS '映射类型: industry/style/product';
COMMENT ON COLUMN decoration_keyword_mapping.mapping_value IS '映射值';
COMMENT ON COLUMN decoration_keyword_mapping.theme_code IS '关联主题编码';
COMMENT ON COLUMN decoration_keyword_mapping.weight IS '匹配权重';
COMMENT ON COLUMN decoration_keyword_mapping.match_count IS '匹配次数';
COMMENT ON COLUMN decoration_keyword_mapping.status IS '状态：0禁用 1启用';
...
ADD COLUMN IF NOT EXISTS current_step INT DEFAULT NULL,
ADD COLUMN IF NOT EXISTS selected_industry VARCHAR(50) DEFAULT NULL,
ADD COLUMN IF NOT EXISTS selected_style VARCHAR(50) DEFAULT NULL,
ADD COLUMN IF NOT EXISTS selected_theme_code VARCHAR(50) DEFAULT NULL,
ADD COLUMN IF NOT EXISTS selected_layout_id BIGINT DEFAULT NULL,
ADD COLUMN IF NOT EXISTS guide_data TEXT DEFAULT NULL;

COMMENT ON COLUMN ai_decoration_session.current_step IS '当前引导步骤：1-4';
COMMENT ON COLUMN ai_decoration_session.selected_industry IS '选择的行业类型';
COMMENT ON COLUMN ai_decoration_session.selected_style IS '选择的风格类型';
COMMENT ON COLUMN ai_decoration_session.selected_theme_code IS '选择的主题编码';
COMMENT ON COLUMN ai_decoration_session.selected_layout_id IS '选择的布局ID';
COMMENT ON COLUMN ai_decoration_session.guide_data IS '引导过程中间数据JSON';
