...
<script setup name="GoodsSpuEdit">
import { ref, reactive, computed, onMounted } from 'vue'
import { useRouter, useRoute } from 'vue-router'
import { ElMessage } from 'element-plus'
import { Plus, Delete } from '@element-plus/icons-vue'
import { getObj, putObj } from '@/api/mall/goodsspu'
import { listBySpuId, saveTiers } from '@/api/mall/goodspricetier'
...
const loadGoodsData = async () => {
  ...
  try {
    ...
    if (data.categoryFirst) {
      const categoryArr = [data.categoryFirst]
      if (data.categorySecond) {
        categoryArr.push(data.categorySecond)
      }
      form.categoryId = categoryArr
    }

    // 确保shelf是字符串
    form.shelf = String(data.shelf || '0')

    // 处理图片数组
    if (typeof data.picUrls === 'string') {
      try {
        form.picUrls = JSON.parse(data.picUrls)
      } catch (e) {
        form.picUrls = data.picUrls ? [data.picUrls] : []
      }
    } else if (Array.isArray(data.picUrls)) {
      form.picUrls = data.picUrls
    }
...
// 适配侧边栏收起状态
.hideSidebar .goods-edit .footer-actions {
  left: 54px;
}
</style>
