...
const FarmingRecordView = ({ 
  ...
  // 状态配置
  const statusConfig = {
    healthy: { variant: 'success', text: '健康', color: '#52C41A' },
    sick: { variant: 'error', text: '生病', color: '#FF4D4F' },
    treatment: { variant: 'warning', text: '治疗中', color: '#FA8C16' },
    quarantine: { variant: 'warning', text: '隔离', color: '#FA8C16' },
    recovered: { variant: 'success', text: '康复', color: '#52C41A' },
    dead: { variant: 'error', text: '死亡', color: '#FF4D4F' },
    normal: { variant: 'default', text: '正常', color: '#9CA3AF' }
  };
  ...
  const getFarmingIcon = (animalType) => {
    return farmingIcons[animalType] || farmingIcons.default;
  };
        ...
        render: (value, record) => (
          <div className="flex items-center">
            <div className="w-8 h-8 bg-[#E6F7FF] rounded-full flex items-center justify-center mr-3">
              <i className={`${getFarmingIcon(record.animalType)} text-[#1890FF] text-sm`}></i>
            </div>
            <div>
              <div className="font-medium">{value || '未知编号'}</div>
...
// 养殖记录卡片组件 - 严格遵循UI设计系统规则
const FarmingRecordCard = ({ 
  record, 
  expanded, 
  onExpand, 
  onClick, 
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center flex-1">
          <div className="w-10 h-10 bg-[#E6F7FF] rounded-full flex items-center justify-center mr-3">
            <i className={`${getFarmingIcon(record.animalType)} text-[#1890FF]`}></i>
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-medium text-gray-900 truncate">
              {record.animalId || '未知编号'}
            </h3>
            <p className="text-sm text-gray-600 truncate">
              {record.animalType || '未知类型'} • ID: {record.id}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <span className="ml-1 font-medium">{record.operation || '-'}</span>
        </div>
        <div>
          <span className="text-gray-600">年龄:</span>
          <span className="ml-1 font-medium">{record.age || '-'}</span>
        </div>
        {showLocation && (
          <div>
          ...
          <div className="space-y-2 text-sm">
            ...
            {record.weight && (
              <div>
                ...
                <span className="ml-1">{record.weight}kg</span>
              </div>
            )}
            {record.temperature && (
              <div>
...
// 养殖记录详情组件 - 严格遵循UI设计系统规则
const FarmingRecordDetail = ({ 
  record, 
  showTimeline, 
  getFarmingIcon, 
  getStatusInfo, 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm p-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            <div className="w-12 h-12 bg-[#E6F7FF] rounded-full flex items-center justify-center mr-4">
              <i className={`${getFarmingIcon(record.animalType)} text-[#1890FF] text-xl`}></i>
            </div>
            <div>
              <h1 className="text-lg font-medium text-gray-900">
                {record.animalId || '未知编号'}
              </h1>
              <p className="text-sm text-gray-600">{record.animalType || '未知类型'} • ID: {record.id}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-gray-50 p-3 rounded-lg">
            ...
            <div className="font-medium">{record.operation || '-'}</div>
          </div>
          <div className="bg-gray-50 p-3 rounded-lg">
      ...
      {(record.weight || record.temperature || record.heartRate) && (
        <div className="bg-white rounded-lg shadow-sm p-4">
          <h3 className="text-lg font-medium text-gray-900 mb-3">健康指标</h3>
          <div className="grid grid-cols-2 gap-4">
            {record.weight && (
              <div className="bg-gray-50 p-3 rounded-lg">
            ...
            {record.heartRate && (
              <div className="bg-gray-50 p-3 rounded-lg">
                <div className="text-sm text-gray-600">心率</div>
                <div className="font-medium">{record.heartRate}次/分</div>
              </div>
            )}
          </div>
        </div>
      ...
      {showTimeline && record.timeline && (
        <div className="bg-white rounded-lg shadow-sm p-4">
          <h3 className="text-lg font-medium text-gray-900 mb-4">操作时间线</h3>
          <div className="space-y-4">
...
export default FarmingRecordView; 