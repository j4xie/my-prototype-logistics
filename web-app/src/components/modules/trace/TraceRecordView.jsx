...
const TraceRecordView = ({ 
  ...
  const statusConfig = {
    ...
    verified: { variant: 'success', text: '已验证', color: '#52C41A' },
    pending: { variant: 'warning', text: '待处理', color: '#FA8C16' },
    review: { variant: 'warning', text: '审核中', color: '#FA8C16' },
    error: { variant: 'error', text: '错误', color: '#FF4D4F' },
    rejected: { variant: 'error', text: '已拒绝', color: '#FF4D4F' },
    draft: { variant: 'default', text: '草稿', color: '#9CA3AF' }
  };

  // 产品图标配置
  const productIcons = {
    meat: 'fas fa-drumstick-bite',
    poultry: 'fas fa-kiwi-bird',
    dairy: 'fas fa-cheese',
    fish: 'fas fa-fish',
    fruit: 'fas fa-apple-alt',
    vegetable: 'fas fa-carrot',
    grain: 'fas fa-seedling',
    default: 'fas fa-box'
  };
        ...
        render: (value, record) => (
          <div className="flex items-center">
            <div className="w-8 h-8 bg-[#E6F7FF] rounded-full flex items-center justify-center mr-3">
              <i className={`${getProductIcon(record.productType)} text-[#1890FF] text-sm`}></i>
            </div>
            <div>
              <div className="font-medium">{value || '未知产品'}</div>
...
// 溯源记录卡片组件 - 严格遵循UI设计系统规则
const TraceRecordCard = ({ 
  record, 
  expanded, 
  onExpand, 
  onClick, 
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center flex-1">
          <div className="w-10 h-10 bg-[#E6F7FF] rounded-full flex items-center justify-center mr-3">
            <i className={`${getProductIcon(record.productType)} text-[#1890FF]`}></i>
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <span className="ml-1 font-medium">{record.batchNumber || '-'}</span>
        </div>
        <div>
          <span className="text-gray-600">阶段:</span>
          <span className="ml-1 font-medium">{record.stage || '-'}</span>
        </div>
        {showLocation && (
          <div>
...
// 溯源记录详情组件 - 严格遵循UI设计系统规则
const TraceRecordDetail = ({ 
  record, 
  showTimeline, 
  getProductIcon, 
  getStatusInfo, 
      ...
      <div className="bg-white rounded-lg shadow-sm p-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            <div className="w-12 h-12 bg-[#E6F7FF] rounded-full flex items-center justify-center mr-4">
              <i className={`${getProductIcon(record.productType)} text-[#1890FF] text-xl`}></i>
            </div>
            <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-gray-50 p-3 rounded-lg">
            <div className="text-sm text-gray-600">批次号</div>
          ...
          <div className="bg-gray-50 p-3 rounded-lg">
            ...
            <div className="font-medium">{record.stage || record.operation || '-'}</div>
          </div>
          <div className="bg-gray-50 p-3 rounded-lg">
