...
function buildFlowFromSM(states: SMState[], transitions: SMTransition[]) {
  ...
  nodes.value = states.map((s, i) => ({
    ...
    data: {
      ...
      nodeSchemaType: (s as Record<string, unknown>).nodeSchemaType || '',
      nodeCategory: (s as Record<string, unknown>).nodeCategory || '',
      nodeConfig: (s as Record<string, unknown>).nodeConfig || {},
      assignedRoles: (s as Record<string, unknown>).assignedRoles || [],
      timeLimitMinutes: (s as Record<string, unknown>).timeLimitMinutes || 0,
      autoEscalate: (s as Record<string, unknown>).autoEscalate || false,
      entryGuard: (s as Record<string, unknown>).entryGuard || '',
      onEnterAction: (s as Record<string, unknown>).onEnterAction || '',
      onExitAction: (s as Record<string, unknown>).onExitAction || '',
    },
  }))
  ...
  edges.value = transitions.map((t, i) => ({
    ...
    data: {
      ...
      notifyRoles: (t as Record<string, unknown>).notifyRoles || [],
    },
  }))
}
