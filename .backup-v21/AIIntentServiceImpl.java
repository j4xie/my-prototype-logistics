...
import com.cretas.aims.service.IntentMatchRecordService;
...
public class AIIntentServiceImpl implements AIIntentService {
     ...
     * 异步保存匹配记录，不阻塞主流程
     */
    private final IntentMatchRecordService intentMatchRecordService;
    ...
    /**
     * v16.1: 是否启用 CLASSIFIER + SEMANTIC 并行融合
     * 当启用时，CLASSIFIER 和 SEMANTIC 并行执行并融合结果
     */
    @Value("${cretas.ai.fusion.enabled:true}")
     ...
     * 融合结果低于此阈值时继续后续流程
     */
    @Value("${cretas.ai.fusion.min-confidence:0.60}")
    private double fusionMinConfidence;

    /**
     * v16.1: SEMANTIC 优先选择阈值
     * 当 SEMANTIC 置信度 >= 此值且 CLASSIFIER 置信度 < fusionClassifierLowThreshold 时，选择 SEMANTIC
     */
    @Value("${cretas.ai.fusion.semantic-prefer-threshold:0.85}")
    private double fusionSemanticPreferThreshold;

    /**
     * v16.1: CLASSIFIER 低置信度阈值
     * 用于判断是否应该优先选择 SEMANTIC 结果
     */
    @Value("${cretas.ai.fusion.classifier-low-threshold:0.60}")
    private double fusionClassifierLowThreshold;

    /**
     * v16.1: 并行执行超时时间（毫秒）
     */
    @Value("${cretas.ai.fusion.timeout-ms:500}")
    private long fusionTimeoutMs;
    ...
    public IntentMatchResult recognizeIntentWithConfidence(String userInput, String factoryId, int topN,
        ...
        if (!skipPhraseShortcut) {
            // v18.0: 使用带置信度梯度的短语匹配
            // 先用原始输入匹配（保留完整上下文），未命中再用预处理后输入（去掉了语气词"呢"/"吧"/"啊"）
            Optional<IntentKnowledgeBase.PhraseMatchResult> earlyPhraseMatch = knowledgeBase.matchPhraseWithConfidence(userInput);
            if (!earlyPhraseMatch.isPresent() && !processedInput.equals(userInput)) {
                earlyPhraseMatch = knowledgeBase.matchPhraseWithConfidence(processedInput);
            }
            if (earlyPhraseMatch.isPresent()) {
                IntentKnowledgeBase.PhraseMatchResult phraseMatchResult = earlyPhraseMatch.get();
                String matchedIntentCode = phraseMatchResult.getIntentCode();
                double phraseConfidence = phraseMatchResult.getConfidence();
                ...
                if (hasConflict) {
                    ...
                    // v17.0: 先检测 ActionType，用于修正意图
                    ActionType detectedActionType = knowledgeBase.detectActionType(userInput.toLowerCase().trim());

                    // v17.0: ActionType 感知修正
                    String correctedIntent = correctIntentByActionType(matchedIntentCode, detectedActionType);
                    if (!correctedIntent.equals(matchedIntentCode)) {
                        log.info("v17.0 ActionType 修正: {} -> {} (action={})", matchedIntentCode, correctedIntent, detectedActionType);
                    }
                    // v20.0: 时间修饰词触发 STATS 修正
                    correctedIntent = correctListToStatsByTimeModifier(correctedIntent, userInput);
                    final String finalIntentCode = correctedIntent;

                    List<AIIntentConfig> allIntents = getAllIntents(factoryId);
                    Optional<AIIntentConfig> intentOpt = allIntents.stream()
                            .filter(i -> i.getIntentCode().equals(finalIntentCode))
                            .findFirst();
                    ...
                    if (intentOpt.isPresent()) {
                        ...
                        // ========== v21.0: 歧义信号检测 + 置信度校准 ==========
                        // 检测可能导致 PhraseMatch 出错的歧义信号
                        IntentKnowledgeBase.AmbiguitySignals ambiguity =
                                knowledgeBase.detectAmbiguitySignals(userInput, finalIntentCode);

                        // v21.0 增强：检测 v17.0 修正导致的意图变化
                        // 如果 v17.0 把 CREATE/UPDATE 改成了 LIST/QUERY，这可能是误判
                        if (!matchedIntentCode.equals(finalIntentCode)) {
                            // 检查是否从操作类意图被改成了查询类意图
                            boolean originalIsAction = matchedIntentCode.contains("CREATE") ||
                                    matchedIntentCode.contains("UPDATE") ||
                                    matchedIntentCode.contains("DELETE") ||
                                    matchedIntentCode.contains("START");
                            boolean finalIsQuery = finalIntentCode.contains("LIST") ||
                                    finalIntentCode.contains("QUERY") ||
                                    finalIntentCode.contains("STATS");

                            if (originalIsAction && finalIsQuery) {
                                // v17.0 可能误判，添加歧义信号
                                ambiguity.addSignal(
                                        IntentKnowledgeBase.AmbiguityType.ACTION_CONFLICT,
                                        0.30,  // 较高的惩罚
                                        "v17.0 intent correction: " + matchedIntentCode + " -> " + finalIntentCode);
                                log.info("v21.0 检测到v17.0修正冲突: {} -> {}, 添加歧义信号",
                                        matchedIntentCode, finalIntentCode);
                            }
                        }

                        double calibratedConfidence = phraseConfidence;
                        if (ambiguity.hasAmbiguity()) {
                            // 应用惩罚，但不低于 0.55 (LLM Fallback 阈值附近)
                            calibratedConfidence = Math.max(0.55, phraseConfidence - ambiguity.getTotalPenalty());
                            log.info("v21.0 歧义检测: intent={}, 原置信度={}, 校准后={}, 信号={}",
                                    finalIntentCode, String.format("%.2f", phraseConfidence),
                                    String.format("%.2f", calibratedConfidence), ambiguity);
                        }

                        // v21.0: 校准后置信度仍然很高，直接返回
                        if (calibratedConfidence >= 0.85) {
                            log.info("v18.0 PhraseMatch shortcut: input='{}', intent={}, confidence={}, phrase='{}'",
                                    userInput, matchedIntentCode, String.format("%.2f", calibratedConfidence),
                                    phraseMatchResult.getMatchedPhrase());
                            ...
                            IntentMatchResult phraseResult = IntentMatchResult.builder()
                                    ...
                                    .topCandidates(Collections.singletonList(CandidateIntent.fromConfig(
                                            intent, calibratedConfidence, (int) (calibratedConfidence * 100),
                                            Collections.emptyList(), MatchMethod.PHRASE_MATCH)))
                                    .confidence(calibratedConfidence)
                                    .matchMethod(MatchMethod.PHRASE_MATCH)
                                    .matchedKeywords(Collections.emptyList())
                                    .isStrongSignal(calibratedConfidence >= 0.92)
                        ...
                        // v21.0: 校准后置信度在中等区间 (0.58-0.85)，触发 LLM Reranking
                        if (calibratedConfidence >= matchingConfig.getLlmRerankingLowerBound()) {
                            log.info("v21.0 置信度降级触发 LLM Reranking: intent={}, calibrated={}, original={}",
                                    finalIntentCode, String.format("%.2f", calibratedConfidence),
                                    String.format("%.2f", phraseConfidence));

                            // 构建候选列表用于 LLM Reranking
                            List<CandidateIntent> candidates = new ArrayList<>();
                            candidates.add(CandidateIntent.fromConfig(
                                    intent, calibratedConfidence, (int) (calibratedConfidence * 100),
                                    Collections.emptyList(), MatchMethod.PHRASE_MATCH));

                            // 添加可能的替代意图作为候选
                            // 基于检测到的 ActionType 和歧义类型，添加相关候选
                            addAlternativeCandidatesForReranking(candidates, userInput, finalIntentCode,
                                    detectedActionType, ambiguity, allIntents);
                            ...
                            // 调用 LLM Reranking
                            return tryLlmReranking(userInput, factoryId, candidates, phraseResult,
                                    detectedActionType, userId, userRole);
                        }

                        // v21.0: 校准后置信度很低 (< 0.58)，不返回 PhraseMatch 结果
                        // 继续执行后续 CLASSIFIER + SEMANTIC 融合流程
                        log.info("v21.0 置信度过低，跳过 PhraseMatch: intent={}, calibrated={}",
                                finalIntentCode, String.format("%.2f", calibratedConfidence));
                    }
                }
            }
        ...
        // ========== v16.1: CLASSIFIER + SEMANTIC 并行融合 ==========
        // 同时执行 BERT 分类器和语义相似度匹配，融合两者结果
        // 融合策略:
        //   - 两者意图一致: 直接采纳，confidence = max(classifier, semantic)
        //   - 两者意图不一致:
        //     - 若 semantic >= 0.85 且 classifier < 0.60: 采纳 SEMANTIC
        //     - 否则采纳 CLASSIFIER (BERT 决策边界更清晰)
        if (fusionEnabled && !skipPhraseShortcut
                && classifierEnabled && classifierIntentMatcher != null && classifierIntentMatcher.isAvailable()) {
            try {
                long fusionStartTime = System.currentTimeMillis();
                final String inputForClassifier = processedInput;

                // 并行执行 CLASSIFIER
                CompletableFuture<Optional<ClassifierResult>> classifierFuture =
                        CompletableFuture.supplyAsync(() -> {
                            try {
                                return classifierIntentMatcher.classify(inputForClassifier);
                            } catch (Exception e) {
                                log.warn("v16.1 Classifier async error: {}", e.getMessage());
                                return Optional.empty();
                            }
                        });

                // 并行执行 SEMANTIC (如果可用)
                CompletableFuture<SemanticMatchResult> semanticFuture = null;
                if (semanticSimilarityEnabled && semanticIntentMatcher != null
                        && semanticIntentMatcher.isSemanticMatchingAvailable()) {
                    final String inputForSemantic = userInput;
                    semanticFuture = CompletableFuture.supplyAsync(() -> {
                        try {
                            // 使用较低阈值 0.70，让更多结果进入融合判断
                            return semanticIntentMatcher.matchBySimilarity(inputForSemantic, 0.70);
                        } catch (Exception e) {
                            log.warn("v16.1 Semantic async error: {}", e.getMessage());
                ...
                // 等待结果（带超时）
                Optional<ClassifierResult> classifierOpt = classifierFuture.get(fusionTimeoutMs, TimeUnit.MILLISECONDS);
                SemanticMatchResult semanticResult = (semanticFuture != null)
                        ? semanticFuture.get(fusionTimeoutMs, TimeUnit.MILLISECONDS)
                        : SemanticMatchResult.noMatch();

                long fusionLatency = System.currentTimeMillis() - fusionStartTime;

                // 融合决策
                IntentMatchResult fusedResult = fuseClassifierAndSemantic(
                        classifierOpt.orElse(null), semanticResult, userInput, processedInput,
                        factoryId, preprocessedQuery, fusionLatency);

                if (fusedResult != null) {
                    saveIntentMatchRecord(fusedResult, factoryId, userId, sessionId, false);
                    return applyNegationConversion(fusedResult, enhancedResult, factoryId);
                }

                // 融合结果置信度不足，继续后续流程
                log.debug("v16.1 Fusion confidence insufficient, proceeding to semantic router");

            } catch (TimeoutException e) {
                log.warn("v16.1 Parallel fusion timeout ({}ms), falling back to sequential", fusionTimeoutMs);
            } catch (Exception e) {
                log.warn("v16.1 Parallel fusion error: {}, falling back to semantic router", e.getMessage());
            }
        }

        // ========== v16.0 Fallback: 串行 CLASSIFIER (当融合未启用或失败时) ==========
        // 使用 BERT 模型直接分类，准确率 97.45% Top-1
        // 高置信度直接返回，低置信度继续走语义路由器
        if (!fusionEnabled && classifierEnabled && classifierIntentMatcher != null && classifierIntentMatcher.isAvailable()
                && !skipPhraseShortcut) {
            try {
                ...
                if (classifierResult.isPresent()) {
                    ...
                    if (result.getConfidence() >= classifierHighConfidenceThreshold) {
                        // v18.0: REPORT 子类别后修正
                        String reclassifiedIntent = reclassifyReportIntent(result.getIntentCode(), userInput);
                        if (!reclassifiedIntent.equals(result.getIntentCode())) {
                            log.info("v18.0 Serial CLASSIFIER reclassify: {} -> {}", result.getIntentCode(), reclassifiedIntent);
                        }
                        // v20.0: 时间修饰词触发 STATS 修正
                        final String finalIntent = correctListToStatsByTimeModifier(reclassifiedIntent, userInput);

                        List<AIIntentConfig> allIntents = getAllIntents(factoryId);
                        Optional<AIIntentConfig> intentOpt = allIntents.stream()
                                .filter(i -> i.getIntentCode().equals(finalIntent))
                                .findFirst();
                        ...
                        if (intentOpt.isPresent()) {
                            ...
                            log.info("v16.0 CLASSIFIER_DIRECT: intent={}, confidence={}, saved LLM call",
                                    reclassifiedIntent, String.format("%.4f", result.getConfidence()));
    ...
    private IntentMatchResult doRecognizeIntentWithConfidence(String processedInput, String originalInput,
        ...
        if (originalPhraseMatch.isPresent() && !skipPhraseShortcut) {
            ...
            // v17.0: ActionType 感知修正
            String correctedIntent = correctIntentByActionType(matchedIntent, detectedActionType);
            if (!correctedIntent.equals(matchedIntent)) {
                log.info("v17.0 ActionType 修正: {} -> {} (action={})", matchedIntent, correctedIntent, detectedActionType);
            }
            // v20.0: 时间修饰词触发 STATS 修正
            correctedIntent = correctListToStatsByTimeModifier(correctedIntent, originalInput);
            final String finalMatchedIntent = correctedIntent;

            List<AIIntentConfig> allIntents = getAllIntents(factoryId);
            Optional<AIIntentConfig> intentOpt = allIntents.stream()
                    .filter(i -> i.getIntentCode().equals(finalMatchedIntent))
                    .findFirst();
        ...
        if (!processedInput.equals(originalInput) && !skipPhraseShortcut) {
            ...
            if (processedPhraseMatch.isPresent()) {
                ...
                // v20.0: 时间修饰词触发 STATS 修正
                correctedIntent = correctListToStatsByTimeModifier(correctedIntent, processedInput);
                final String finalMatchedIntent = correctedIntent;
        ...
        // ========== v14.0 Layer 0.4: 语义相似度匹配 (Standalone Fallback) ==========
        // 当精确短语匹配失败时，使用向量相似度进行泛化匹配
        // 例如: "销量最高" 语义匹配到 "销售排名" -> REPORT_KPI
        // v14.0修复: 当检测到多意图触发词时，跳过此快速路径
        // v16.1: 当融合启用时跳过此独立匹配（已在融合逻辑中处理）
        if (!fusionEnabled && !skipPhraseShortcut && semanticSimilarityEnabled && semanticIntentMatcher != null
                && semanticIntentMatcher.isSemanticMatchingAvailable()) {
            try {
    ...
    /**
     * v21.0: 为 LLM Reranking 添加替代候选意图
     *
     * <p>当检测到歧义信号时，根据歧义类型添加可能的替代意图作为候选，
     * 让 LLM 从多个候选中选择最合适的意图</p>
     *
     * @param candidates 候选列表 (会被修改)
     * @param userInput 用户输入
     * @param matchedIntent 当前匹配的意图
     * @param detectedAction 检测到的 ActionType
     * @param ambiguity 歧义信号
     * @param allIntents 所有可用意图
     */
    private void addAlternativeCandidatesForReranking(
        ...
        addedIntents.add(matchedIntent);

        // 根据 ActionType 添加相关意图
        String prefix = extractIntentPrefix(matchedIntent);

        if (detectedAction == ActionType.CREATE) {
            // 如果检测到 CREATE 动作，添加同领域的 CREATE 意图
            addIntentByPattern(candidates, allIntents, prefix + "_CREATE", 0.65, addedIntents);
            addIntentByPattern(candidates, allIntents, prefix + "_BATCH_CREATE", 0.60, addedIntents);
            addIntentByPattern(candidates, allIntents, prefix + "_ADD", 0.60, addedIntents);
        } else if (detectedAction == ActionType.UPDATE) {
            // UPDATE 动作
            addIntentByPattern(candidates, allIntents, prefix + "_UPDATE", 0.65, addedIntents);
            addIntentByPattern(candidates, allIntents, prefix + "_MODIFY", 0.60, addedIntents);
            addIntentByPattern(candidates, allIntents, prefix + "_STATUS_UPDATE", 0.60, addedIntents);
        } else if (detectedAction == ActionType.QUERY) {
            // QUERY 动作 - 添加同领域的查询变体
            addIntentByPattern(candidates, allIntents, prefix + "_LIST", 0.65, addedIntents);
            addIntentByPattern(candidates, allIntents, prefix + "_QUERY", 0.65, addedIntents);
            addIntentByPattern(candidates, allIntents, prefix + "_STATS", 0.60, addedIntents);
        }

        // 检查歧义类型，添加特定的替代意图
        for (IntentKnowledgeBase.AmbiguitySignal signal : ambiguity.getSignals()) {
            if (signal.getType() == IntentKnowledgeBase.AmbiguityType.TIME_WITH_LIST) {
                // 时间 + LIST 歧义，添加 STATS 变体
                String statsIntent = matchedIntent.replace("_LIST", "_STATS");
                addIntentByCode(candidates, allIntents, statsIntent, 0.70, addedIntents);
            } else if (signal.getType() == IntentKnowledgeBase.AmbiguityType.ACTION_CONFLICT) {
                // ActionType 冲突，添加对应 ActionType 的意图
                if (detectedAction == ActionType.CREATE && matchedIntent.endsWith("_LIST")) {
                    String createIntent = matchedIntent.replace("_LIST", "_CREATE");
                    addIntentByCode(candidates, allIntents, createIntent, 0.70, addedIntents);
                }
            }
        }
        ...
        if (candidates.size() > 5) {
            candidates.subList(5, candidates.size()).clear();
        }

        log.debug("v21.0 添加替代候选: original={}, candidates={}",
                matchedIntent, candidates.stream().map(c -> c.getIntentCode()).collect(java.util.stream.Collectors.toList()));
    }

    /**
     * v21.0: 提取意图代码的前缀 (领域部分)
     */
    private String extractIntentPrefix(String intentCode) {
        if (intentCode == null) return "";
        // 移除常见后缀
        String[] suffixes = {"_LIST", "_QUERY", "_STATS", "_CREATE", "_UPDATE", "_DELETE", "_DETAIL", "_INFO"};
     ...
     * v21.0: 根据模式添加意图到候选列表
     */
    private void addIntentByPattern(List<CandidateIntent> candidates, List<AIIntentConfig> allIntents,
                                     String pattern, double confidence, Set<String> addedIntents) {
        for (AIIntentConfig intent : allIntents) {
            String code = intent.getIntentCode().toUpperCase();
            if (code.equals(pattern) && !addedIntents.contains(code)) {
                candidates.add(CandidateIntent.fromConfig(
                        intent, confidence, (int)(confidence * 100),
                        Collections.emptyList(), MatchMethod.PHRASE_MATCH));
                addedIntents.add(code);
                break;
            }
        }
    }
     ...
     * v21.0: 根据意图代码添加到候选列表
     */
    private void addIntentByCode(List<CandidateIntent> candidates, List<AIIntentConfig> allIntents,
                                  String intentCode, double confidence, Set<String> addedIntents) {
        if (intentCode == null || addedIntents.contains(intentCode.toUpperCase())) {
            return;
        }
        for (AIIntentConfig intent : allIntents) {
            if (intent.getIntentCode().equalsIgnoreCase(intentCode)) {
                candidates.add(CandidateIntent.fromConfig(
                        intent, confidence, (int)(confidence * 100),
                        Collections.emptyList(), MatchMethod.PHRASE_MATCH));
                addedIntents.add(intentCode.toUpperCase());
                break;
            }
        }
    }
    ...
    /**
     * v17.0: 基于 ActionType 修正意图
     *
     * 当短语匹配得到的意图与检测到的 ActionType 冲突时，修正为正确的意图。
     * 例如：
     * - 输入 "创建原料批次"，短语匹配到 MATERIAL_BATCH_QUERY，但 ActionType=CREATE
     *   -> 修正为 MATERIAL_BATCH_CREATE
     * - 输入 "查询质检记录"，短语匹配到 QUALITY_CHECK_EXECUTE，但 ActionType=QUERY
     *   -> 修正为 QUALITY_CHECK_QUERY
     *
     * @param matchedIntent 短语匹配得到的意图代码
     * @param actionType 检测到的操作类型
     * @return 修正后的意图代码（如果无需修正则返回原值）
     */
    private String correctIntentByActionType(String matchedIntent, ActionType actionType) {
        if (matchedIntent == null || actionType == null) {
            return matchedIntent;
        }

        // 只对明确的操作类型进行修正
        if (actionType == ActionType.AMBIGUOUS || actionType == ActionType.UNKNOWN) {
            return matchedIntent;
        }

        // v17.1: 如果意图已经明确是查询类 (QUERY/LIST/STATS等)，不要因为 ActionType=CREATE 而覆盖
        // 原因: ActionType 检测可能因为 "发货"、"入库" 等词被误判为 CREATE
        // 但短语匹配如 "查看发货情况" → SHIPMENT_QUERY 是更准确的
        // v18.0: 扩展保护标记，防止 START/PAUSE/RESUME/COMPLETE/CANCEL 被 CREATE 覆盖
        // v19.0: START 可以修正 LIST（如 "看看启动批次" 应该是 START 而非 LIST）
        Set<String> protectedMarkers = Set.of(
                "QUERY", "LIST", "STATS", "HISTORY", "OVERVIEW", "STATUS",
                "START", "PAUSE", "RESUME", "COMPLETE", "CANCEL"
        );
        boolean intentIsProtected = protectedMarkers.stream().anyMatch(matchedIntent::contains);
        // v19.0: 只保护 CREATE 误判，START 可以修正 LIST
        if (intentIsProtected && actionType == ActionType.CREATE) {
            log.debug("v18.0 跳过修正: 意图 {} 受保护，ActionType=CREATE 可能是误判", matchedIntent);
            return matchedIntent;
        }
        // v19.0: START 可以修正 LIST 意图（如 "启动批次" 匹配到 PROCESSING_BATCH_LIST，应修正为 START）
        if (matchedIntent.contains("LIST") && actionType == ActionType.START) {
            log.debug("v19.0 START 修正: 意图 {} 应修正为 START 版本", matchedIntent);
            // 继续执行修正逻辑
        }

        // 定义意图族映射：同一实体的不同操作
        Map<String, Map<ActionType, String>> intentFamilies = new HashMap<>();

        // 原料批次族
        intentFamilies.put("MATERIAL_BATCH", Map.of(
                ActionType.CREATE, "MATERIAL_BATCH_CREATE",
                ActionType.QUERY, "MATERIAL_BATCH_QUERY"
        ));

        // v17.2: 质检族 - 移除，因为 "看看质量检查" ActionType=QUERY 会错误覆盖 EXECUTE
        // QUALITY_CHECK 的 EXECUTE vs QUERY 由短语映射精确控制

        // 发货/出货族
        intentFamilies.put("SHIPMENT", Map.of(
                ActionType.CREATE, "SHIPMENT_CREATE",
                ActionType.QUERY, "SHIPMENT_QUERY"
        ));

        // 告警族
        intentFamilies.put("ALERT", Map.of(
                ActionType.QUERY, "ALERT_LIST"
        ));

        // 加工批次族 — v18.0: 移除 CREATE 映射，CREATE/START 由短语精确控制
        // v19.0: 完善 PROCESSING_BATCH 家族，支持 START/CREATE/QUERY 区分
        Map<ActionType, String> processingBatchFamily = new HashMap<>();
        processingBatchFamily.put(ActionType.QUERY, "PROCESSING_BATCH_LIST");
        processingBatchFamily.put(ActionType.START, "PROCESSING_BATCH_START");
        processingBatchFamily.put(ActionType.CREATE, "PROCESSING_BATCH_CREATE");
        intentFamilies.put("PROCESSING_BATCH", processingBatchFamily);
        ...
        // 查找意图族映射
        Map<ActionType, String> familyMap = intentFamilies.get(intentFamily);
        if (familyMap == null) {
            return matchedIntent;
        }

        // 检查是否需要修正
        String correctedIntent = familyMap.get(actionType);
        if (correctedIntent != null && !correctedIntent.equals(matchedIntent)) {
     ...
     *
     * 例如：
     * - MATERIAL_BATCH_CREATE -> MATERIAL_BATCH
     * - QUALITY_CHECK_EXECUTE -> QUALITY_CHECK
     * - SHIPMENT_QUERY -> SHIPMENT
     *
     * @param intentCode 意图代码
     * @return 意图族名称，如果无法提取则返回 null
     */
    private String extractIntentFamily(String intentCode) {
        if (intentCode == null) {
            return null;
        }

        // 操作后缀列表
        // v18.0: 扩展操作后缀，支持 START/PAUSE/RESUME/COMPLETE/CANCEL
        String[] actionSuffixes = {
                "_CREATE", "_QUERY", "_UPDATE", "_DELETE",
                "_EXECUTE", "_LIST", "_DETAIL", "_SEARCH",
                "_START", "_PAUSE", "_RESUME", "_COMPLETE", "_CANCEL"
        };

        String upperIntent = intentCode.toUpperCase();
        for (String suffix : actionSuffixes) {
            if (upperIntent.endsWith(suffix)) {
                return upperIntent.substring(0, upperIntent.length() - suffix.length());
            }
        }
    ...
    /**
     * v18.0: REPORT 子类别后修正
     * CLASSIFIER 倾向把所有 REPORT 查询归到 DASHBOARD_OVERVIEW，
     * 用关键词检测修正为具体子类别
     */
    private String reclassifyReportIntent(String intent, String input) {
        if (!"REPORT_DASHBOARD_OVERVIEW".equals(intent)) {
            return intent;
        }
        String lower = input.toLowerCase();
        if (lower.contains("财务") || lower.contains("利润") || lower.contains("成本利润") || lower.contains("收支") || lower.contains("盈亏")) {
            return "REPORT_FINANCE";
        }
        if (lower.contains("趋势") || lower.contains("走势") || lower.contains("变化趋势")) {
            return "REPORT_TRENDS";
        }
        if (lower.contains("异常") && (lower.contains("报") || lower.contains("分析") || lower.contains("汇总"))) {
            return "REPORT_ANOMALY";
        }
        if (lower.contains("效率") || lower.contains("oee") || lower.contains("利用率")) {
            return "REPORT_EFFICIENCY";
        }
        if (lower.contains("kpi") || lower.contains("绩效") || lower.contains("指标")) {
            return "REPORT_KPI";
        }
        if (lower.contains("客户") && (lower.contains("分析") || lower.contains("统计"))) {
            return "CUSTOMER_STATS";
        }
        if (lower.contains("费用") && lower.contains("统计")) {
            return "COST_QUERY";
        }
        return intent;
    }
    ...
    /**
     * v20.0: 时间修饰词触发 LIST→STATS 修正
     * 当用户输入包含时间修饰词（昨天、本月、最近）时，
     * 将 LIST 意图修正为对应的 STATS 版本
     *
     * 例如：
     * - "设备故障最近" + EQUIPMENT_ALERT_LIST → EQUIPMENT_ALERT_STATS
     * - "考勤情况本月" + ATTENDANCE_TODAY → ATTENDANCE_STATS
     *
     * @param intent 匹配到的意图
     * @param input 用户输入
     * @return 修正后的意图
     */
    private String correctListToStatsByTimeModifier(String intent, String input) {
        ...
        // 时间修饰词：触发统计类意图
        boolean hasTimeModifier = lower.contains("昨天") || lower.contains("前天") ||
                lower.contains("本月") || lower.contains("上月") || lower.contains("这个月") ||
                lower.contains("本周") || lower.contains("上周") || lower.contains("这周") ||
                lower.contains("今年") || lower.contains("去年") ||
                lower.contains("最近") || lower.contains("近期") || lower.contains("这段时间") ||
                lower.contains("统计") || lower.contains("汇总") || lower.contains("分析");
        ...
        // LIST → STATS 映射
        Map<String, String> listToStatsMapping = Map.of(
                "EQUIPMENT_ALERT_LIST", "EQUIPMENT_ALERT_STATS",
                "ALERT_LIST", "ALERT_STATS",
                "ATTENDANCE_TODAY", "ATTENDANCE_STATS",
                "ATTENDANCE_HISTORY", "ATTENDANCE_STATS",
                "QUALITY_CHECK_QUERY", "QUALITY_STATS",
                "SHIPMENT_QUERY", "SHIPMENT_STATS"
        );

        String statsIntent = listToStatsMapping.get(intent);
        if (statsIntent != null) {
            log.info("v20.0 时间修饰词修正: {} -> {} (input='{}')", intent, statsIntent, input);
            return statsIntent;
        }
     ...
     * 保存意图匹配记录（异步，不阻塞主流程）
     *
     * Phase 1 重构: 使用 IntentMatchRecordService 异步保存
     * - 异步保存后自动回填 recordId 到 result
     * - 支持后续反馈关联
     *
     * @param result 匹配结果
     * @param factoryId 工厂ID
    ...
    private void saveIntentMatchRecord(IntentMatchResult result, String factoryId,
        ...
        // 使用专用线程池异步保存，不阻塞主流程
        CompletableFuture<String> recordIdFuture = intentMatchRecordService.saveRecordAsync(
                result, factoryId, userId, sessionId, llmCalled);

        // 异步回填 recordId，供后续反馈使用
        recordIdFuture.thenAccept(recordId -> {
            if (recordId != null && result != null) {
                result.setRecordId(recordId);
                log.debug("Intent match recordId set: {}", recordId);
            }
        }).exceptionally(ex -> {
            log.warn("Failed to set recordId: {}", ex.getMessage());
            return null;
        });
    }
    ...
    /**
     * v16.1: 融合 CLASSIFIER 和 SEMANTIC 结果
     *
     * 融合策略:
     * 1. 两者意图一致: 直接采纳，confidence = max(classifier, semantic)
     * 2. 两者意图不一致:
     *    - 若 semantic >= fusionSemanticPreferThreshold 且 classifier < fusionClassifierLowThreshold: 采纳 SEMANTIC
     *    - 否则采纳 CLASSIFIER (BERT 决策边界更清晰)
     * 3. 融合结果置信度 < fusionMinConfidence 时返回 null，继续后续流程
     *
     * @param classifier        CLASSIFIER 结果（可为 null）
     * @param semantic          SEMANTIC 结果（可为 noMatch）
     * @param userInput         用户原始输入
     * @param processedInput    预处理后的输入
     * @param factoryId         工厂ID
     * @param preprocessedQuery 预处理查询结果
     * @param fusionLatency     融合耗时（毫秒）
     * @return 融合后的 IntentMatchResult，如果置信度不足返回 null
     */
    private IntentMatchResult fuseClassifierAndSemantic(
            ClassifierResult classifier,
            SemanticMatchResult semantic,
            String userInput,
            String processedInput,
            String factoryId,
        ...
        boolean hasClassifier = classifier != null && classifier.getConfidence() > 0;
        boolean hasSemantic = semantic != null && semantic.isMatched();

        if (!hasClassifier && !hasSemantic) {
            log.debug("v16.1 Fusion: both CLASSIFIER and SEMANTIC returned no result");
            return null;
        }
        ...
        if (hasClassifier && hasSemantic) {
            // 两者都有结果
            String classifierIntent = classifier.getIntentCode();
            String semanticIntent = semantic.getIntentCode();

            if (classifierIntent.equals(semanticIntent)) {
                // 一致: 取较高置信度
                finalIntent = classifierIntent;
                // v18.0: REPORT 子类别后修正
                finalIntent = reclassifyReportIntent(finalIntent, userInput);
                // v20.0: 时间修饰词触发 STATS 修正
                finalIntent = correctListToStatsByTimeModifier(finalIntent, userInput);
                finalConfidence = Math.max(classifier.getConfidence(), semantic.getSimilarity());
                matchMethod = MatchMethod.CLASSIFIER; // 标记为 CLASSIFIER (融合一致)
                matchedPhrase = semantic.getMatchedPhrase();
                log.info("v16.1 Fusion AGREE: intent={}, classifier={}, semantic={}, final={}, latency={}ms",
                        finalIntent,
                        String.format("%.4f", classifier.getConfidence()),
                        String.format("%.4f", semantic.getSimilarity()),
                        String.format("%.4f", finalConfidence),
                        fusionLatency);
            } else {
                // 不一致: 基于阈值决策
                if (semantic.getSimilarity() >= fusionSemanticPreferThreshold
                        && classifier.getConfidence() < fusionClassifierLowThreshold) {
                    // SEMANTIC 高置信度且 CLASSIFIER 低置信度，选择 SEMANTIC
                    finalIntent = semanticIntent;
                    // v20.0: 时间修饰词触发 STATS 修正
                    finalIntent = correctListToStatsByTimeModifier(finalIntent, userInput);
                    finalConfidence = semantic.getSimilarity();
                    matchMethod = MatchMethod.SEMANTIC;
                    matchedPhrase = semantic.getMatchedPhrase();
                    log.info("v16.1 Fusion DISAGREE -> SEMANTIC: sem_intent={}, sem_conf={}, cls_intent={}, cls_conf={}, latency={}ms",
                            semanticIntent,
                            String.format("%.4f", semantic.getSimilarity()),
                            classifierIntent,
                            String.format("%.4f", classifier.getConfidence()),
                    ...
                    // v20.0: 时间修饰词触发 STATS 修正
                    finalIntent = correctListToStatsByTimeModifier(finalIntent, userInput);
                    finalConfidence = classifier.getConfidence();
                    matchMethod = MatchMethod.CLASSIFIER;
                    log.info("v16.1 Fusion DISAGREE -> CLASSIFIER: cls_intent={}, cls_conf={}, sem_intent={}, sem_conf={}, latency={}ms",
                            classifierIntent,
                            String.format("%.4f", classifier.getConfidence()),
                            semanticIntent,
                            String.format("%.4f", semantic.getSimilarity()),
            ...
            // 只有 CLASSIFIER 结果
            finalIntent = classifier.getIntentCode();
            // v20.0: 时间修饰词触发 STATS 修正
            finalIntent = correctListToStatsByTimeModifier(finalIntent, userInput);
            finalConfidence = classifier.getConfidence();
            matchMethod = MatchMethod.CLASSIFIER;
            log.info("v16.1 Fusion CLASSIFIER_ONLY: intent={}, confidence={}, latency={}ms",
                    finalIntent, String.format("%.4f", finalConfidence), fusionLatency);
        } else {
            // 只有 SEMANTIC 结果
            finalIntent = semantic.getIntentCode();
            finalConfidence = semantic.getSimilarity();
            matchMethod = MatchMethod.SEMANTIC;
            matchedPhrase = semantic.getMatchedPhrase();
            log.info("v16.1 Fusion SEMANTIC_ONLY: intent={}, similarity={}, phrase='{}', latency={}ms",
                    finalIntent, String.format("%.4f", finalConfidence), matchedPhrase, fusionLatency);
        }

        // 置信度阈值检查
        if (finalConfidence < fusionMinConfidence) {
            log.debug("v16.1 Fusion confidence {} < threshold {}, skipping",
                    String.format("%.4f", finalConfidence), fusionMinConfidence);
            return null;
        }

        // 查找意图配置
        final String resolvedIntent = finalIntent; // effectively final for lambda
        List<AIIntentConfig> allIntents = getAllIntents(factoryId);
        Optional<AIIntentConfig> intentOpt = allIntents.stream()
                .filter(i -> i.getIntentCode().equals(resolvedIntent))
                .findFirst();

        if (!intentOpt.isPresent()) {
            log.warn("v16.1 Fusion intent not found in config: {}", resolvedIntent);
            return null;
        }
        ...
        // 构建匹配关键词列表
        List<String> matchedKeywords = matchedPhrase != null
                ? Collections.singletonList(matchedPhrase)
                : Collections.emptyList();

        IntentMatchResult result = IntentMatchResult.builder()
                .bestMatch(intent)
                .topCandidates(Collections.singletonList(CandidateIntent.fromConfig(
                        intent, finalConfidence,
                        (int) (finalConfidence * 100),
                        matchedKeywords, matchMethod)))
                .confidence(finalConfidence)
                .matchMethod(matchMethod)
                .matchedKeywords(matchedKeywords)
                .isStrongSignal(finalConfidence >= 0.85)
                .requiresConfirmation(finalConfidence < 0.85)
                .userInput(userInput)
                .actionType(detectedActionType)
                .build();
        ...
        log.info("v16.1 FUSION_RESULT: method={}, intent={}, confidence={}, latency={}ms",
                matchMethod, resolvedIntent, String.format("%.4f", finalConfidence), fusionLatency);
