 ...
 * MobileNav组件属性
 */
export interface MobileNavProps extends Omit<React.HTMLAttributes<HTMLElement>, 'onSelect'> {
  /** 导航项列表 */
  items?: NavItem[];
  /** 当前活动项 */
  activeItem?: string;
...
const MobileNav = forwardRef<HTMLElement, MobileNavProps>(({
  ...
  // 处理项目点击
  const handleItemClick = useCallback((item: NavItem, index: number) => {
  ...
  // 获取导航项样式
  const getNavItemClasses = useCallback((item: NavItem, index: number, isActive: boolean) => {
...
const BottomTabBar = forwardRef<HTMLDivElement, BottomTabBarProps>(({
  ...
  return (
    <div 
      ref={ref}
      className={cn('fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-[999]', className)}
      {...props}
    >
      <div className="max-w-[390px] mx-auto">
        <div className="flex justify-around py-2">
          {tabs.map((tab) => (
            <button
              ...
              className={cn(
                'flex flex-col items-center py-2 px-3 min-w-0 flex-1 transition-colors relative',
                activeTab === tab.key
                  ? 'text-[#1890FF]'
                  : tab.disabled
                  ? 'text-gray-400 cursor-not-allowed'
