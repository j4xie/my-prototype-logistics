...
export default function CreateFactoryModal({ onSuccess, onCancel }: CreateFactoryModalProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.factory_name.trim()) {
      newErrors.factory_name = '请输入工厂名称';
    }
    if (!formData.industry.trim()) {
      newErrors.industry = '请输入所属行业';
    }
    if (!formData.contact_address.trim()) {
      newErrors.contact_address = '请输入工厂地址';
    }
    if (!formData.owner_name.trim()) {
      newErrors.owner_name = '请输入联系人姓名';
    }
    if (!formData.owner_phone.trim()) {
      newErrors.owner_phone = '请输入联系电话';
    } else if (!/^1[3-9]\d{9}$/.test(formData.owner_phone)) {
      newErrors.owner_phone = '请输入正确的手机号码';
    }
    if (!formData.owner_email.trim()) {
      newErrors.owner_email = '请输入联系邮箱';
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.owner_email)) {
      newErrors.owner_email = '请输入正确的邮箱地址';
    }
    if (formData.employee_count < 1) {
      newErrors.employee_count = '员工数量必须大于0';
    }
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={handleInputChange('factory_name')}
                    placeholder="请输入工厂名称"
                    className={errors.factory_name ? 'border-red-500' : ''}
                  />
                  {errors.factory_name && (
                    <p className="text-sm text-red-500 mt-1">{errors.factory_name}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {errors.contact_address && (
                    <p className="text-sm text-red-500 mt-1">{errors.contact_address}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {errors.employee_count && (
                    <p className="text-sm text-red-500 mt-1">{errors.employee_count}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleInputChange('subscription_plan')}
