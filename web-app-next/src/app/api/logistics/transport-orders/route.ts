...
// Original file backed up and disabled on: 2025-06-04T04:13:25.855Z
...
// transport-orders/[id] API路由
// 生成时间: 2025-06-03T14:10:37.483Z
// 模块: logistics
// 实体: TransportOrder
...
export async function GET(_request: NextRequest) {
  try {
    ...
    console.error('GET /logistics/transport-orders/[id] error:', error);
    return NextResponse.json({
      success: false,
...
export async function PUT(_request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: updatedData,
      message: 'TransportOrder updated successfully',
      timestamp: new Date().toISOString()
    });

  } catch (error) {
    console.error('PUT /logistics/transport-orders/[id] error:', error);
    return NextResponse.json({
      success: false,
      message: 'Internal Server Error',
      timestamp: new Date().toISOString()
...
export async function DELETE(_request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: { id },
      message: 'TransportOrder deleted successfully',
      timestamp: new Date().toISOString()
    });

  } catch (error) {
    console.error('DELETE /logistics/transport-orders/[id] error:', error);
    return NextResponse.json({
      success: false,
      message: 'Internal Server Error',
      timestamp: new Date().toISOString()
