...
// Original file backed up and disabled on: 2025-06-04T04:13:25.861Z
...
// harvest-records/[id] API路由
// 生成时间: 2025-06-03T14:10:37.460Z
// 模块: farming
// 实体: HarvestRecord
...
export async function GET(_request: NextRequest) {
  try {
    ...
    console.error('GET /farming/harvest-records/[id] error:', error);
    return NextResponse.json({
      success: false,
...
export async function PUT(_request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: updatedData,
      message: 'HarvestRecord updated successfully',
      timestamp: new Date().toISOString()
    });

  } catch (error) {
    console.error('PUT /farming/harvest-records/[id] error:', error);
    return NextResponse.json({
      success: false,
      message: 'Internal Server Error',
      timestamp: new Date().toISOString()
...
export async function DELETE(_request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: { id },
      message: 'HarvestRecord deleted successfully',
      timestamp: new Date().toISOString()
    });

  } catch (error) {
    console.error('DELETE /farming/harvest-records/[id] error:', error);
    return NextResponse.json({
      success: false,
      message: 'Internal Server Error',
      timestamp: new Date().toISOString()
