...
// Original file backed up and disabled on: 2025-06-04T04:13:25.866Z
...
// planting-plans/[id] API路由
// 生成时间: 2025-06-03T14:10:37.453Z
// 模块: farming
// 实体: PlantingPlan
...
export async function GET(_request: NextRequest) {
  try {
    ...
    console.error('GET /farming/planting-plans/[id] error:', error);
    return NextResponse.json({
      success: false,
...
export async function PUT(_request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: updatedData,
      message: 'PlantingPlan updated successfully',
      timestamp: new Date().toISOString()
    });

  } catch (error) {
    console.error('PUT /farming/planting-plans/[id] error:', error);
    return NextResponse.json({
      success: false,
      message: 'Internal Server Error',
      timestamp: new Date().toISOString()
...
export async function DELETE(_request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: { id },
      message: 'PlantingPlan deleted successfully',
      timestamp: new Date().toISOString()
    });

  } catch (error) {
    console.error('DELETE /farming/planting-plans/[id] error:', error);
    return NextResponse.json({
      success: false,
      message: 'Internal Server Error',
      timestamp: new Date().toISOString()
