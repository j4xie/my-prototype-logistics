...
export const environments: Record<string, MockEnvironment> = {
  development: {
    ...
    enabled: true,
    handlers: ['auth', 'users', 'farming', 'processing', 'logistics', 'admin', 'trace'],
    dataSet: 'full',
    delay: [100, 600],
  ...
  test: {
    ...
    baseUrl: 'http://localhost:3000/api',
    enabled: true,
    handlers: ['auth', 'users', 'farming', 'processing'],
    dataSet: 'minimal',
    delay: [0, 50],
...
export const mockMiddleware = {
  shouldMock: (): boolean => {
    ...
    // 检查全局Mock开关
    const mockEnabled = process.env.NEXT_PUBLIC_MOCK_ENABLED !== 'false'
