"""V2: Use finer scene threshold + min gap 3s, include dense frames as fallback in long gaps."""
...
def main() -> None:
    ...
    timestamps = [
        float(x.strip())
        for x in (base / "scene_fine_timestamps.txt").read_text(encoding="utf-8-sig").splitlines()
        if x.strip()
    ]
    frames = sorted((base / "frames_scene_fine").glob("f_*.jpg"))
    ...
    for ts, frame_path in zip(timestamps, frames):
        ...
        storyboard.append({
            "ts": ts,
            "time": fmt(ts),
            "frame": f"keyframes_v2/{new_name}",
            "caption": caption_at(ts),
            "source": "scene",
        })

    # Plug long gaps (>= 25s) with dense frames from frames_dense/
    dense_frames = sorted((base / "frames_dense").glob("d_*.jpg"))
    DENSE_INTERVAL = 3.0  # dense was sampled every 3s
    ...
    (base / "storyboard_v2.json").write_text(
        json.dumps(augmented, ensure_ascii=False, indent=2), encoding="utf-8"
    )

    md = [
        f"# Storyboard V2 ({len(augmented)} 帧, threshold=0.05, min_gap=3s, gap-fill from dense)\n",
        f"原始场景帧: {len(frames)}, 去重后: {len(storyboard)}, 加 gap-fill 后: {len(augmented)}\n\n---\n",
    ]
    for it in augmented:
        tag = " 🌫️" if it["source"] == "dense (gap-fill)" else ""
        md.append(f"### {it['time']}{tag} — `{it['frame']}`\n\n> {it['caption']}\n")
    (base / "storyboard_v2.md").write_text("\n".join(md), encoding="utf-8")
